...
namespace Automatic
{
    public partial class ProForm : Form
    {
        ...
        msnmp cable1 = new msnmp();
        msnmp cableZB = new msnmp();
        ...
        double Dup2 = 0;
        double waveP2 = 0;
        double noisevalued = 0;
        double CountFAIL = 0;
        //bool getSSIDName_lan1 = false;
        ...
        bool infomationMIBPass = false;
        bool Result_CheckLed_EthGreen;
        bool accessWebsiteNewPassCon = false;
        bool getWANMACPass;
        bool enableZigBeePass;
        bool disaZigBeePass;
        bool disaZigBeeFail = false;
        ...
        bool CheckLed_OnlineFail = true;
        bool CheckLed_WPSPass = false;
        bool DKGet_SN = false;
        bool checkPingMocaPass;
        bool ConfigFirst = false;
        string Ytext, Xtext;
        string LbMACText;
        string LbMTAText;
        string LbSN;
        ...
        double StartTimeL_online= 0.00;
        double StartTimeL_USB_C= 0.00;
        double StartTimeL_WPS= 0.00;

        double StartTimeV_voip_1 = 0.00;
        double StartTimeV_noise_1 = 0.00;
        double StartTimeV_voip_2 = 0.00;
        double StartTimeV_noise_2 = 0.00;

        double StartTimeT = 0.00;
        double StopTimeT = 0.00;
        double StartTime = 0.00;

        double Time_Unplus_cable_RF = 0.00;
        double Time_plus_cable_MOCA = 0.00;


        double StopTime = 0.00;
        double StartTimeS = 0.00;
        double StopTimeS = 0.00;
        double StartTimeB = 0.00;
        double StopTimeB = 0.00;
        double StartTimeZ = 0.00;
        double StopTimeZ = 0.00;
        double StartTimeV = 0.00;
        double StopTimeV = 0.00;
        double StartTimeL = 0.00;
        double StopTimeL = 0.00;
        ...
        private double GettimeStartB(string Function)
        {
            string timestart = DateTime.Now.ToString();
            msg(Function + " " + timestart + " START");
            double TimeNow = DateTime.Now.Ticks;///DateTime.Now.Hour * 3600 + DateTime.Now.Minute * 60 + DateTime.Now.Millisecond;
        ...
        private double GettimeEndB(string Function)
        {
            string timeend = DateTime.Now.ToString();
            msg(Function + "  " + timeend + " END");
            double TimeNow = DateTime.Now.Ticks;//DateTime.Now.Hour * 3600 + DateTime.Now.Minute * 60 + DateTime.Now.Second;
        ...
        private double CountTimeZ(string Function, double TimeStart, double TimeEnd)
        {
            double TotalTime = Math.Round((TimeEnd - TimeStart) / 10000000, 2);
            msg("Total time " + Function + ": " + TotalTime);
            StartTimeZ = 0;
        ...
        private double CountTimeB(string Function, double TimeStart, double TimeEnd)
        {
            double TotalTime = Math.Round((TimeEnd - TimeStart) / 10000000, 2);
            msg("Total time " + Function + ": " + TotalTime);
            StartTimeB = 0;
        ...
        private double CountTimeL(string Function, double TimeStart, double TimeEnd)
        {
            double TotalTime = 0;
            if (TimeStart>0)
            {
                TotalTime = Math.Round((TimeEnd - TimeStart) / 10000000, 2);
                msg("Total time " + Function + ": " + TotalTime);
                StartTimeL = 0;
                StopTimeL = 0;
            }
        ...
        private double CountTimeV(string Function, double TimeStart, double TimeEnd)
        {
            double TotalTime = 0;
            if (TimeStart > 0)
            {
        ...
        private double CountTime1(string Function, double TimeStart, double TimeEnd)
        {
            double TotalTime = Math.Round((TimeEnd - TimeStart) / 10000000, 2);
            msg( Function + ": " + TotalTime);
            return TotalTime;
        }
        private bool WebAutoNian()
        {
            ...
            {
                if(!ConfigSSIDPassword())
                {
                    //StopTime = GettimeEnd("GET_INFOR_WEB");
                    //Timeweb = CountTime("GET_INFOR_WEB", StartTime, StopTime);
                    msgFail("GET_INFOR_WEB ======>FAIL");
                    StatusERR("FAIL WEB FIRST"); return false;
                }
            }
            if (!LoginWEBFirst())
            {
                ...
                //Timeweb = CountTime("GET_INFOR_WEB", StartTime, StopTime);
                msgFail("GET_INFOR_WEB ======>FAIL");
                StatusERR("accessWebsite"); return false;
            }
            //StopTime = GettimeEnd("GET_INFOR_WEB");
            //Timeweb = CountTime("GET_INFOR_WEB", StartTime, StopTime);
            //timeTest.Enabled = false;

            StartTime = GettimeStart("Unplus_cable_RF");
            if (sModem == "CGM4981COM")
            {
                Show("", "Xac nhan/Confirm", 20000);
                kq_stust = MyMessagesBox_new1.Show("\n\n              Rut_RF_CABLE!!");

                if (kq_stust != DialogResult.Yes && kq_stust != DialogResult.No)
                {
                    msg("OP time out Unplug RF cable");
                    StopTime = GettimeEnd("Unplus_cable_RF");
                    Time_Unplus_cable_RF = CountTime("Unplus_cable_RF", StartTime, StopTime);
                    StopTime = GettimeEnd("GET_INFOR_WEB");
                    Timeweb = CountTime("GET_INFOR_WEB", StartTime, StopTime);
                    ErrorNian("Err_timeout_OP_unplug_RF_cabel"); return false;
                }
                //MyMessagesBox.Show("\n\n              Rut_RF_CABLE!!");
            }
            else
            ...
            delay(75000);
            if (!checkPingtoDUT_neww("Ping to Dut", "Eth1", " ", "10.0.0.1", 12, 6, mt8))
            {
                if (!checkPingtoDUT_neww("Ping to Dut", "Eth1", " ", "10.0.0.1", 12, 6, mt8))
                {
                    StopTime = GettimeEnd("GET_INFOR_WEB");
                    Timeweb = CountTime("GET_INFOR_WEB", StartTime, StopTime);
                    ErrorNian("Err_Ping_after_unplug_RF_cabel");
                    return false;
                }
            }
            ...
            if (!LoginWithNewPassword())
            {
                //StopTime = GettimeEnd("GET_INFOR_WEB");
                //Timeweb = CountTime("GET_INFOR_WEB", StartTime, StopTime);
                StatusERR("LoginWithNewPassword"); return false;
            }
            if (sModem == "CGM4981COM" | sModem == "CGM4981COX" | sModem == "CGM4981SHW"|sModem == "CGM4981ROG"| sModem == "CGM4981COM-DEV")
            {
                if (!GetMACInforWEBUISW58())
                {
                    //StopTime = GettimeEnd("GET_INFOR_WEB");
                    //Timeweb = CountTime("GET_INFOR_WEB", StartTime, StopTime);
                    StatusERR("GetInforMAC_Address"); return false;
                }
            }
            else
            {
            ...
            if (!GeWifiInforWEBUI())
            {
                //StopTime = GettimeEnd("GET_INFOR_WEB");
                //Timeweb = CountTime("GET_INFOR_WEB", StartTime, StopTime);
                StatusERR("Bootup_Wifi"); return false;
            }
        ...
        public void EthGreen()
        {
            try
            {
                if (!CheckLed_EthGreen())
                { 
                    ErrorNian("Err_Eth1_Led_Green");
                    return;
                }
            }
            catch { ErrorNian("Err_Eth1_Led_Green"); }
        }
        ...
        private bool Checksignal_VOIP()
        {
            ...
            if (thStattus1 == true)
            {
                for (int i = 1; i <= 2; i++)
                {
                    ...
                    {
                        StopTime = GettimeEnd("ETH1");
                        TimepingEth1 = CountTime("ETH1", StartTime, StopTime);
                        Checksignal_VOIPPass = false;
        ...
        public void thrMultledWPS()
        {
            ...
            try
            {
                StartTimeL_WPS = GettimeStart("LED_WPS");
                if (!CheckLed_WPSNian(1)) // faile se vao
                {
                    //StopTimeL = GettimeEnd("LED_WPS");
                    //ledWPS1 = CountTimeL("LED_WPS", StartTimeL, StopTimeL);

                    StartTimeL_online = GettimeStart("LED_Online");
                    if (!CheckLed_OnlineNian())
                    {
                        //if (Type_Program == "on")
                        //{
                            //StopTimeL = GettimeEnd("LED_Online");
                            //ledonline = CountTimeL("LED_Online", StartTimeL, StopTimeL);
                            StatusERR("Fail Led_oline"); return;
                        //}
                    }              
                    delay(5000);
                    StopTimeL = GettimeEnd("LED_Online");
                    ledonline = CountTimeL("LED_Online", StartTimeL_online, StopTimeL);

                    StartTimeL_USB_C = GettimeStart("USB_TYPE_C");
                    //cmmModem.Items.Add("CGM4981COM");
                    if (cmmModem.Text.Contains("CGM4981COM"))
                    {
                        CheckLed_USB = true;
                    }
                    else
                    {
                        if (!CheckLed_USB_C())
                        {
                            //if (Type_Program == "ON")
                            //{
                            StopTimeL = GettimeEnd("USB_TYPE_C");
                            Usb_C1 = CountTimeL("USB_TYPE_C", StartTimeL_USB_C, StopTimeL);
                            ErrorNian("Err_USB_TypeC"); StatusERR("Fail_LED_USB"); return;
                            //}
                        }
                    }
                    ...
                    StopTimeL = GettimeEnd("LED_WPS");
                    ledWPS1 = CountTimeL("LED_WPS", StartTimeL_WPS, StopTimeL);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (!CheckLed_USB_C())
                        {
                            //if (Type_Program == "ON")
                            //{
                            StopTime = GettimeEnd("USB_TYPE_C");
                            Usb_C1 = CountTimeL("USB_TYPE_C", StartTimeL_USB_C, StopTimeL);
                            ErrorNian("Err_USB_TypeC"); StatusERR("Fail_LED_USB"); return;
                            //}
                        }
        ...
        public void thrMultVOIP()
        {
            ...
            {
                voipf = false;
                StartTimeV_voip_1 = GettimeStart("VOIP_12");
                dkresetodefoult = true;
                if (CheckVoipP1toP2PASS == false)
                {
                    //double TimeStart = GettimeStart("VOIP1");
                    Status4("CHECK P1 >> P2 VOIP");
                    if (!CheckVoipP1toP2Nian())
                    {
                        //if (Type_Program == "ON")
                        //{
                            //StopTimeV = GettimeEnd("VOIP_12");
                            //TimeVoip1 = CountTimeV("VOIP_12", StartTimeV, StopTimeV);
                            dkvoipretest = true; StatusERR("CheckVoipP1toP2"); return;
                        //}
                        //dkvoipretest = true;
                    }
                    StopTimeV = GettimeEnd("VOIP_12");
                    TimeVoip1 = CountTimeV("VOIP_12", StartTimeV_voip_1, StopTimeV);

                    StartTimeV_noise_1 = GettimeStart("NOISE_12");
                    if (VoiP1Pass == true)
                    {
                        if (!noiseTest("P1toP2"))
                        {
                            ...
                            if (!noiseTest("P1toP2"))
                            {
                                //if (Type_Program == "ON")
                                //{
                                    StopTimeV = GettimeEnd("NOISE_12");
                                    TimeNoise1 = CountTimeV("NOISE_12", StartTimeV_noise_1, StopTimeV);
                ...
                {
                    StartTimeV_voip_2 = GettimeStart("VOIP_21");
                    Status4("CHECK P2 >> P1 VOIP");
                    if (!CheckVoipP2toP1Nian())
                    {
                        //if (Type_Program == "ON")
                        //{
                            //StopTimeV = GettimeEnd("VOIP_21");
                            //TimeNoise1 = CountTimeV("VOIP_21", StartTimeV, StopTimeV);
                            dkvoipretest = true; StatusERR("CheckVoipP2toP1"); return;
                        //}
                        //dkvoipretest = true;
                    }
                    StopTimeV = GettimeEnd("VOIP_21");
                    TimeVoip2 = CountTimeV("VOIP_21", StartTimeV_voip_2, StopTimeV);

                    StartTimeV_noise_2 = GettimeStart("NOISE_21");
                    if (VoiP2Pass == true)
                    {
                        if (!noiseTest("P2toP1"))
                        {
                            //StopTime = GettimeEnd("NOISE_21");
                            //TimeNoise2 = CountTime("NOISE_21", StartTime, StopTime);
                            dkvoipretest = true;
                            ...
                            if (!noiseTest("P2toP1"))
                            {
                                //if (Type_Program == "ON")
                                //{
                                    StopTimeV = GettimeEnd("NOISE_21");
                                    TimeNoise2 = CountTimeV("NOISE_21", StartTimeV_noise_2, StopTimeV);
        ...
        {   try
            {
                if (thStattus1 == true)
                {
                    StartTimeZ = GettimeStart("EN_ZIGBEE");
                    if (!enableZigbee())
                    {
                        if (Type_Program == "ON")
                        {
                            StatusERR("enableZigbee"); return;
                            //StopTimeZ = GettimeEnd("EN_ZIGBEE");
                            //TimeEnableZgbee = CountTimeZ("EN_ZIGBEE", StartTimeZ, StopTimeZ);
                        }
                    }
                    delay(2000);
                    StopTimeZ = GettimeEnd("EN_ZIGBEE");
                    ...
                    StartTimeZ = GettimeStart("EN_BLE");
                    if (!enableBLE())
                    {
                        if (Type_Program == "ON")
                        {
                            StatusERR("enableBLE"); return;
                            //StopTimeZ = GettimeEnd("EN_BLE");
                            //TimeEnableBLE = CountTimeZ("EN_BLE", StartTimeZ, StopTimeZ);
                        }
                    }
                    StopTimeZ = GettimeEnd("EN_BLE");
                    TimeEnableBLE = CountTimeZ("EN_BLE", StartTimeZ, StopTimeZ);
                    ...
                    //StartTimeZ = GettimeStart("DIS_ZIGBEE");
                    //if (!disableZigbee())
                    //{
                    //    StatusERR("disableZigbee"); return;
                    //    //StopTime = GettimeEnd("EN_BLE");
                    //    //TimeDisableZgbee = CountTime("EN_BLE", StartTime, StopTime);
                    //}
                    //StopTimeZ = GettimeEnd("DIS_ZIGBEE");
                    //TimeDisableZgbee = CountTime("DIS_ZIGBEE", StartTimeZ, StopTimeZ);
                }
                else return;
            }
            catch
        ...
        private void msgFail(string Message)
        {
            try
            {
                if (Failtxt.InvokeRequired)
                {
                    Failtxt.Invoke(new Action(() =>
                    {
                        Failtxt.AppendText(Message + "\r\n");
                    }));
                }
                else
                {
        ...
        private void msgsftp(string Message)
        {
            try
            {
                if (sftptxt.InvokeRequired)
                {
                    sftptxt.Invoke(new Action(() =>
                    {
                        sftptxt.AppendText(Message + "\r\n");
                    }));
                }
                else
                {
        ...
        private void msgPass(string Message)
        {
            ...
            if (Failtxt.InvokeRequired)
            {
                Failtxt.Invoke(new Action(() => Failtxt.AppendText(Message + "  (" + "\"" + "_" + "\"" + ")Pass" + "\r\n")));
            }
            else Failtxt.AppendText(Message + "  (" + "\"" + "_" + "\"" + ")Pass" + "\r\n");
        }     
        ...
        private void LogsftpERR(Exception ERRR)
        {
            try
            {
                string timetest = "";
                ...
                msg(DateTime.Now.ToString());
                date1 = date1.Replace("-", ".");
                string directoryPath = "D:\\LogFilesftpErr" + @"\" + date1 + "\\" + ModelnameFile;
                msg(Nian);
                if (txtOutput.InvokeRequired) { txtOutput.Invoke(new Action(() => Data1 = txtOutput.Text)); }
                else { Data1 = txtOutput.Text; }
                if (!System.IO.Directory.Exists(directoryPath))
                {
                    System.IO.Directory.CreateDirectory(directoryPath);
                }
                ...
                string filePath = directoryPath + @"\" + SNs +  "_" + Type_Program + ".txt";
                pathsaveftp = filePath;
                StreamWriter strWrite = new StreamWriter(filePath, true);
                strWrite.WriteLine(Data1);
                strWrite.Flush();
                strWrite.Close();
            ...
            {
                MessageBox.Show("Không lưu đuợc logsftpErr: " + ex);
            }

        }
        private void LogFile(string passFail, string error, string caset)
        {
            cmn.openExe1(pathfile1, "disableEthAll-SV");
            delay(3000);
            try
            {
                Thread_SFC_new.Abort();
            }
            catch { }
            try
            {
                ...
                date = date.Replace("-", ".");
                DUTresult = passFail;
                StopTimeT = GettimeEnd("TotalTimeTest");
                TotalTest_Time = CountTime1 ("TotalTimeTest", StartTimeT, StopTimeT);
                string directoryPath = "D:\\LogFile" + @"\" + date + "\\" + ModelnameFile;
                msg(Nian);
                // huyen sua
                if (!cmmType.Text.Contains("Offline_Program"))
                {
                    double a = Check_system_fpy(sModem, Computer_name);
                    msg("FPY: " + a.ToString());
                    if (a < 85)
                    {
                        try
                        {
                            string path = Application.StartupPath + "\\" + "ERROR_LOCK.txt";
                            File.Delete(path);
                        }
                        catch { }
                        Delay(1000);
                        write_result_test_pre("ERROR_LOCK.txt", "FPY: " + a.ToString());
                    }
                ...
                cmn.openExe1(pathfile1, "disableEthAll-SV");
                delay(1000);
                thMutiple7 = new Thread(new ThreadStart(Trsftp));
                thMutiple7.IsBackground = true;
                thMutiple7.Start();
                for (int d =0;d<51;d++)
                {
                    if (sendsftp == true | d==50)
                    {
                        break;
                    }
                    delay(1000);
                ...
                {
                    msg("Khong luu duoc logfile ftp");
                }
                try
                {
                    double total_time = 0;
                    total_time = Convert.ToDouble(time11);
                    //time11
                    //  LogFile("FAIL", ERR_CODE,"");
                    //  LogFile("PASS", "", "");
                    Save_Time_test_all(sModem, sns, passFail, error, total_time);
                }
                catch (Exception exc)
                {
                    msg("Khong gui duoc thoi gian test: " + exc);
                }
                if (passFail.Contains("PASS"))
                {
                    comCmd("open_on");
            ...
            {
                MessageBox.Show("Khong gui duoc time test. Exception:" + ex.Message);
        ...
        private void Save_Time_test(string SnDUT, string reult, string error, string time)
        {
            try
            {
                ...
                var weatherForecast = new jsonNian
                {
                    ...
                    FunctionTime = new List<functionTime>
            {
                new functionTime{name ="INITIAL_SETUP",testTime=Setup_first},
                ...
                new functionTime{name ="Get_INFOR_MIB",testTime=(GetIFMIB + GetIFMIB_2)},
                new functionTime{name ="GET_INFOR_WEB",testTime=Timeweb},
                new functionTime{name ="USB_C",testTime=(Usb_C1 + Usb_C2)},
                    ...
                    TestData = new List<testData>
            {
                ...
                new testData{itemName ="Online_Distance",itemValue=Distance_Online},//Distance_Online
        ...
        private void Save_Time_test_all(string modelName, string snDUT, string result, string err_code, double totalTest_Time)
        {
            APIProcess api = new APIProcess();
            api.URL = "https://10.228.110.91/e-alt-api/v1/test/testtimetracking/post"; // ConfigurationSettings.AppSettings["APIgetCheckIns"].ToString();

            Product pd = new Product();
            pd.ModelName = modelName;
            pd.Sn = snDUT;
            pd.Route = "OBA_FUNCTION";
            pd.Station = Computer_name;
            pd.Result = result;
            pd.ErrorCode = err_code;
            pd.TotalTime = totalTest_Time;
            pd.FunctionTime = new List<TimeTest>();
            TimeTest tt = new TimeTest();
            pd.TestData = new List<DataTest>();
            DataTest dt = new DataTest();
            /**/
            tt.Name = "INITIAL_SETUP";
            tt.TestTime = Setup_first;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "ETH1";
            tt.TestTime = TimepingEth1;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "ETH2";
            tt.TestTime = TimepingEth2;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "ETH3";
            tt.TestTime = TimepingEth3;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "ETH4";
            tt.TestTime = TimepingEth4;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "Get_INFOR_MIB";
            tt.TestTime = (GetIFMIB + GetIFMIB_2);
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "LABEL";
            ...
            tt = new TimeTest();
            tt.Name = "LED_Online";
            tt.TestTime = ledonline;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "USB_TYPE_C";
            tt.TestTime = Usb_C1 + Usb_C2;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "LED_WPS";
            tt.TestTime = ledWPS1 + ledWPS2;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "VOIP_12";
            tt.TestTime = TimeVoip1;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "VOIP_21";
            tt.TestTime = TimeVoip2;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "NOISE_12";
            tt.TestTime = TimeNoise1;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "NOISE_21";
            tt.TestTime = TimeNoise2;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "SPEED";
            tt.TestTime = TimeSpeed1G;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "ZIGBEE";
            tt.TestTime = TimeEnableZgbee ;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "BLUETOOTH";
            tt.TestTime = TimeEnableBLE;
            pd.FunctionTime.Add(tt);
            /**/

            tt = new TimeTest();
            tt.Name = "GET_INFOR_WEB";
            tt.TestTime = (Math.Abs(Timeweb - Time_Unplus_cable_RF));
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "UNPLUG_RF";
            tt.TestTime = Time_Unplus_cable_RF;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "MOCA";
            tt.TestTime = (Math.Abs(TimeMoca - Time_plus_cable_MOCA));
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "PLUG_MOCA";
            tt.TestTime = Time_plus_cable_MOCA;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "RESET";
            tt.TestTime = FactoryRST;
            pd.FunctionTime.Add(tt);
            /**/
            tt = new TimeTest();
            tt.Name = "TEST_T_TIME";
            tt.TestTime = totalTest_Time;
            pd.FunctionTime.Add(tt);
            ...
            dt.ItemValue = waveP2;
            pd.TestData.Add(dt);
            noisevalued = noisevalue1;
            dt = new DataTest();
            dt.ItemName = "Noise";
            dt.ItemValue = noisevalued;
            pd.TestData.Add(dt);
            ...
            string ret = api.PostMethod(pd);
            ResultAPI res = JsonConvert.DeserializeObject<ResultAPI>(ret);
            if (res.status == 200)
            {
                txtOutput.AppendText("\nPASS (" + DateTime.Now.ToString() + ": " + res.data + ")");
                txtOutput.AppendText("\r\n" + ret);
            }
            else
            {
                txtOutput.AppendText("\nFAIL (" + DateTime.Now.ToString() + ": " + res.data + ")");
                txtOutput.AppendText("\r" + ret);
            }
        }
        ...
        //private bool Save_logfile_sftp(string fath)
        //{
        //    bool sftp = false;
        //    var client = new SftpClient("200.168.104.98", 22, "sftp", "tenpi");
        //    return false;
        //}
        private bool Save_logfile_SFTP(string modelName, string fordel_path)
        {
            ...
            using (SftpClient clinet = new SftpClient("200.168.104.98", 22, "sftp", "tenpi"))
            {
                ...
                {
                    msg("Send logfie SFTP server ==> FAIL");
                }
                clinet.Disconnect();
            }
            return huyen_dz;
        }
        private void Save_logfile_SFTP1(string modelName, string fordel_path)
        {
            //bool result = false;
            //ModelnameFile = sModem;
            ...
            using (SftpClient clinet = new SftpClient("200.168.104.98", 22, "sftp", "tenpi"))
            {
                ...
                for (int sftp = 1; sftp < 5; sftp++)
                {
                    ...
                    {
                        msgsftp("KHÔNG CONNECT ĐC SFTP :" + ex + "/n" + sftp);
                        if (sftp >= 2)
                        {
                            msgsftp("KHÔNG CONNECT ĐC SFTP/n ===== Bao TQE =====/n" + ex);
                            LogsftpERR(ex);
                            connect1 = false;
                            //return connect1;
                            break;
                ...
                if (connect1 == true)
                {
                    ...
                    {
                        //MessageBox.Show("Can not send logfile to SFTP ===> CALL TQE\n Không tạo đuợc logfile lên SFTP ===> GỌi TQE");
                        msgsftp("Create Directory_SFTP ==> FAIL/n =========Bao TQE====== /n" + ex);
                        LogsftpERR(ex);
                    }
                    for (int Up = 0; Up < 4; Up++)
                    {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            msgsftp("upload logfie SFTP server :" + ex + Up);
                            if (Up == 3)
                            {
                                msgsftp("upload logfie SFTP server/n   ===== Bao TQE =====");
                                LogsftpERR(ex);
                                break;
                            }
                        }
        ...
        private void LogFilePassNianTetst(string noteop)
        {
            try
            {
                string time = "";
                ...
                date = DateTime.Now.ToString("dd-MM-yyyy");
                date = date.Replace("-", ".");
                string date1 = DateTime.Now.ToString("dd.MM.yyyy") + " - " + DateTime.Now.TimeOfDay.Hours.ToString() + "h" + DateTime.Now.TimeOfDay.Minutes.ToString() + "p" + DateTime.Now.TimeOfDay.Seconds.ToString() + "s";
                string directoryPath = "D:\\LogFile" + @"\" + date + "\\" + ModelnameFile;
                string sns = "wxwx";
                if (lbTimecc.InvokeRequired)
                {
                ...
                string filePathSN3 = directoryPath + @"\SummaryTest.ini";
                if (!System.IO.Directory.Exists(directoryPath))
                {
                    System.IO.Directory.CreateDirectory(directoryPath);
                }
                ...
                {
                    StreamWriter strWrite1 = new StreamWriter(filePathSN3, true);
                    strWrite1.WriteLine("Model \tSN \tTime_Test \tTimeAlive \tOP \tNote");
                    strWrite1.Flush();
                    strWrite1.Close();
                    delay(200);
                }
                StreamWriter strWrite = new StreamWriter(filePathSN3, true);
                strWrite.WriteLine(sModem + "\t" + sns + "\t" + time + "\t" + date1 + "\t" + OP + "\t" + noteop + "\t" + Type_Program);      
                strWrite.Flush();
                strWrite.Close();
            }
            catch (Exception ex)
        ...
        private bool loadComport()
        {
            ...
            try
            {
                ...
                try
                {
                    string ComZBBLE11 = loadinfor2.ReadLine();
                    ComZBBLE = getKeyEnd(ComZBBLE11, "COMZBBLE:").Trim();
                }
                catch { }
                loadinfor2.Close();
        ...
        bool huyen = false;
        private bool comCmd2to1_new(string cmd)
        {
            bool a = false;
            DataComVoip = "";
        ...
        private void ErrorNian(string maloi)
        {
            ...
            else 
            {
                ...
                ThreadStart socket_SFC_new = new ThreadStart(SFC_new);
                Thread_SFC_new = new Thread(socket_SFC_new);
                Thread_SFC_new.IsBackground = true;
                Thread_SFC_new.Start();
                Delay(5000);
                int ooo = 4;
                while (true)
                {
                    ...
                    if (time_out_terminal == true)
                    {
                        StatusERR("Connect_SFC_ERR"); //ErrorNian("Err_connect_SFC"); return;
                    }
                    ooo++;
                    Status4("Wait connect SFC: " + ooo.ToString());
                    Delay(1000);
                }
            }
            ...
            //else
            //{
            //    Check_ERRCODEoff(maloi);
            //    CountFAIL++;
            //    //msg(maloi + " ========> FAIL :");
            //    msgFail(maloi + " ========> FAIL :" + ERR_CODE);
            //}
        }
                    ...
                    if (!string.IsNullOrEmpty(cmn.pingdata))
                    {
                        ...
                        else
                        {
                            ...
                            if (countPing >= 70) //30
                            {
                                ...
                                if (o >= 2)
                                {
                                    StopTime = GettimeEnd("RESET");
                                    FactoryRST = CountTime("RESET", StartTime, StopTime);
        ...
        private bool checkPingtoDUT_neww(string PingInformation, string EthCardName, string EthCardNameDisable, string iptest, int pingtimeout, int countPing, string mt)
        {
            bool a = false;
            bool b = true;
            string pathfile1 = Directory.GetCurrentDirectory();
        ...
        private bool checkPingResetToSefoul(string PingInformation, string EthCardName, string EthCardNameDisable, string iptest, int countPing, string mt)
        {
            bool a = false;
            bool b = true;
            string pathfile1 = Directory.GetCurrentDirectory();
        ...
        private bool checkPingtoDUT_cmd(string PingInformation, string mt)
        {
            bool a = false;
            bool b = true;
            cmn.date_TimeStart();
            ...
            while (b)
            {
                ...
                cmd.CmdWrite(mt);
                Delay(4000);
                string data_ping = cmd.CmdRead();
                cmd.CmdWrite("exit");
                msg(data_ping);

                if (!string.IsNullOrEmpty(data_ping))
                {
                    if (data_ping.Contains("(0% loss)") & (data_ping.Contains("TTL=64")))
                    {
                        return true;
                    }
                    else
                    {
                        ping++;
                        cmn.openExe1(pathfile1, "enableEthAll");
                        msg(PingInformation + ": " + ping.ToString());
                        Delay(10000);
                        if (ping > 2)
                        {
                            return false;
        ...
        private string Get_IP_Eth1()
        {
            IPa = "";
            cmd.CmdConnect();
            cmd.CmdWrite("ipconfig | findstr /i " + '"' + "ipv4" + '"');

            Output_get_web = cmd.CmdRead() + "END";
            msg(Output_get_web);
            cmd.CmdWrite("exit");

            string a = common.cut(Output_get_web, "ipconfig | findstr /i " + '"' + "ipv4" + '"', "END");
            string b = common.cut(a, "IPv4 Address. . . . . . . . . . . :", "D:\\").Replace("\n", "").Replace("\r", "").Trim();

            string aa = b.Replace("IPv4 Address. . . . . . . . . . . :", "$");
            string aaa;
            if (aa.Contains("Autoconfiguration IPv4 Address. . :"))
            {
                aaa = aa.Replace("Autoconfiguration IPv4 Address. . :", "$");
            }
            else
            {
                aaa = aa;
            ...
            {
                // msg(i);
                if (i.Contains("192.168.0."))
                {
                    if (!i.Contains("192.168.0.20") && !i.Contains("192.168.0.30") && !i.Contains("192.168.0.40") && !i.Contains("169.254"))
                    {
                        msg("IP_ETH1: " + i.Trim());
                        IPa = i.Trim();
                        break;
                ...
                else if (i.Contains("10.0.0."))
                {
                    if (i.Trim() != "10.0.0.20" && !i.Contains("10.0.0.30") && !i.Contains("10.0.0.40") && !i.Contains("169.254"))
                    {
                        msg("IP_ETH1: " + i.Trim());
                        IPa = i.Trim();
                        break;
        ...
        private bool checkPingEth1_new(string ipp)
        {
            bool a = false;
            double TimeEnd = 0;
            ...
            if (thStattus1 == true)
            {
                if (checkPingtoDUT_cmd("Ping to Eth1", "ping -S " + ipp.Trim() + " "+ InfoIP))
                {
                    checkPingEth2Pass = true;
                    msgPass("Ping_Eth1");
                    //TimeEnd = GettimeEnd("Ping_Eth1");
        ...
        private bool checkPingEth2_new()
        {
            bool a = false;
            double TimeEnd = 0;
            ...
            if (thStattus1 == true)
            {
                if (checkPingtoDUT_cmd("Ping to Eth2", "ping -S "+iptestEth2+" "+ InfoIP))
                {
                    checkPingEth2Pass = true;
                    msgPass("Ping_Eth2");
                    //TimeEnd = GettimeEnd("Ping_Eth2");
                ...
                {
                    StopTime = GettimeEnd("ETH2");
                    TimepingEth2 = CountTime("ETH2", StartTime, StopTime);
                    Status4(err.Err_Eth2_Ping);
        ...
        private bool checkPingEth3_new()
        {
            bool a = false;
            double TimeEnd = 0;
            ...
            if (thStattus1 == true)
            {
                if (checkPingtoDUT_cmd("Ping to Eth3", "ping -S "+ iptestEth3 + " "+ InfoIP))
                {
                    checkPingEth3Pass = true;
                    msgPass("Ping_Eth3");
                    //TimeEnd = GettimeEnd("Ping_Eth3");
                ...
                {
                    StopTime = GettimeEnd("ETH3");
                    TimepingEth3 = CountTime("ETH3", StartTime, StopTime);
                    Status4(err.Err_Eth3_Ping);
        ...
        private bool checkPingEth4_new()
        {
            bool a = false;
            double TimeEnd = 0;
            double TimeStart = GettimeStart("Ping_Eth4");
            Status4("checkPingEth4");
            checkPingEth4Pass = false;
            string pathfile1 = Directory.GetCurrentDirectory();
            if (thStattus1 == true)
            {
                if (checkPingtoDUT_cmd("Ping to Eth4", "ping -S "+ iptestEth4 + " "+ InfoIP))
                {
                    checkPingEth4Pass = true;
                    msgPass("Ping_Eth4");
                    //TimeEnd = GettimeEnd("Ping_Eth4");
                ...
                {
                    StopTime = GettimeEnd("ETH4");
                    TimepingEth4 = CountTime("ETH4", StartTime, StopTime);
                    Status4(err.Err_Eth4_Ping);
        ...
        private bool checkPingMocaMian()
        {
            ...
            cmn.openExe1(pathfile1, "enableEth1");
            delay(3000);
            if (thStattus1 == true && enableMocaWebPss ==true)
            {
                bool b = false;
                bool c = true;
                string ipmoca = "";
                for (int i = 0; i <= 10; i++)
                {
                    ...
                    {                        
                        ...
                        else
                        {
                            ...
                            if (count1 >= 4)
                            {
                                StopTime = GettimeEnd("MOCA");
                                TimeMoca = CountTime("MOCA", StartTime, StopTime);
        ...
        private bool checkPingResetTodefault()
        {
           // set_speed_Eth_port("All", "1000");
            ...
            if (thStattus1 == true)
            {
                ...
                _set_net_enable();
                if (checkPingResetToSefoul("Ping Reset Todefault", "EthAll", "", InfoIP, 10, mt8))
                {
                    ...
                    while (count_Waitchecklabel < 200)
                    {
                        if (Type_Program == "OFF")
                        {
                            if (ConfigFirst == false | accessWebsiteNewPassCon == false)
                            {
                                msgFail("Check_LABEL ====> FAIL");
                                Waitchecklabel = true;
                                break;
                            }
                        }
                        if (nk == true && Type_Program == "ON") return false ;
                    ...
                    if (Waitchecklabel == true) //allfunctionpass
                    {
                        ...
                        else
                        {
                            ...
                            while (true)
                            {
                                ...
                                if (time_out_terminal == true)
                                {
                                    StopTime = GettimeEnd("RESET");
                                    FactoryRST = CountTime("RESET", StartTime, StopTime);
                                    StatusERR("Connect_SFC_ERR"); ErrorNian("Err_connect_SFC"); return false;
                                }
                                ooo++;
                                Status4("Wait connect SFC: " + ooo.ToString());
                                Delay(1000);
                        ...
                        for (int i = 0; i < 6; i++)
                        {
                            ...
                            if (Send_To_SFC_PASS())
                            {
                                ...
                                //    {
                                //        fails++;
                                //        lblerr.Text = "Production ========> FAIL";
                                //        msg("Summary_Test : " + Failtxt.Text);
                                //        lbFail.Text = fails.ToString();
                                //        LogFileFail("FAIL: ",CountFAIL.ToString());
                                //        LogFile("FAIL", CountFAIL.ToString(), "");
                                //        FailOKOK();
                                //        WriteoffP("", "FAIL");
                                //    }
                                //    Status("->~Next Product~");
                            ...
                            else
                            {
                                ...
                                delay(2000);
                                msg("Send SFC P/F time: " + i.ToString());
                            }
                        }
                    }
                    else
        ...
        private bool Check_MAC(string key1,string key2)
        {
            bool Nian = false;
            int c = 0;
        ...
        private bool GetInfLable()
        {
            ...
            if (!GetinforDUT())
            {
                StopTimeB = GettimeEndB("LABLE");
                Lable = CountTimeB("LABLE", StartTimeB, StopTimeB);
                ErrorNian("Err_Lable_to_Get"); return false; };
            if (sModem != "CGM4981COX")
            {
                CM = true;
                WAN = true;
                            ...
                            if (SSIDPass == false)
                            {
                                try
                                {
                                    ...
                                    if (sModem == "CGM4981ROG")
                                    {
                                        ...
                                        {
                                            msg("SSID_lable: " + Data[0].Trim() + "  ==>  " + "SSID_MIB: " + SSIDlable1);
                                            msg("SSID_lable1: " + SSIDlable1 + "  ==>  " + "SSID_MIB: " + SSIDCDUT);
                                            SSIDPass = true;
                                            msgPass("Check_SSID");
                                        }
                                        else
        ...
        {
            double TimeStartE = GettimeStart("Led_Eth_Green");
            string file_ledGreen = Labview_part + "\\LedGREEN.txt";
            ...
            double TimeEndE = GettimeEnd("Led_Eth_Green");
            ledgrren = CountTime("Led_Eth_Green", TimeStartE, TimeEndE);
            return CheckLed_1000EthPass;
        }
        private bool CheckLed_Eth_Red()
        {
           double TimeStartE = GettimeStart("Led_RED_Eth");
           bool ResultLedRED = false;
            ...
            double TimeEndE = GettimeEnd("Led_RED_Eth");
            ledred = CountTime("Led_RED_Eth", TimeStartE, TimeEndE);
            return ResultLedRED;
        }
            ...
            for (int i = 1; i <= 2; i++)
            {
                ...
                try
                {
                    ...
                    if (Barcode2D.Length > 10 & !Barcode2D.Contains("Error"))
                    {
                        ...
                        {
                            StopTime = GettimeEnd("INITIAL_SETUP");
                            Setup_first = CountTime("INITIAL_SETUP", StartTime, StopTime);
                            MyMessagesBox.Show("\n\n           OP Chọn sai Model!!!");
                        ...
                        else
                        {
                            ...
                            if (SNlabel.Length > 0)
                            {
                                ...
                                try
                                {
                                    ...
                                    //StreamWriter strWrite1 = new StreamWriter(Path1, true);
                                    //strWrite1.WriteLine(SNlabel);
                                    //strWrite1.Flush();
                                    //strWrite1.Close();
                                    //delay(100);
                                }
                    ...
                    if (!ResultOnline.Equals("")) //&!ResultOnline.Contains("Error")
                    {
                        try
                        {
                            ...
                            try
                            {
                                ...
                                valueRed_Online = Math.Round(Convert.ToDouble(Data[1].Substring(0, Data[1].Length - 2)), 2);
                                valueGreen_Online = Math.Round(Convert.ToDouble(Data[2].Substring(0, Data[2].Length - 2)), 2);
                                valueBlue_Online = Math.Round(Convert.ToDouble(Data[3].Substring(0, Data[3].Length - 2)), 2);
                                valueX_Online = Math.Round(Convert.ToDouble(Data[4].Substring(0, Data[4].Length - 2)), 2);
                                valueY_Online = Math.Round(Convert.ToDouble(Data[5].Substring(0, Data[5].Length - 9)), 2);
                                Distance_Online = Math.Round(Convert.ToDouble(Data[6].Substring(0, Data[6].Length - 2)), 4);
                            }
        ...
        private string GetLanMacNian()
        {
            ...
            while (b)
            {
                ...
                {
                    StopTime = GettimeEnd("Get_INFOR_MIB");
                    GetIFMIB = CountTime("Get_INFOR_MIB", StartTime, StopTime);
        ...
        private void thrLable()//lable
        {
            ...
            {
                //double TimeEnd = 0.0;
                StartTimeB = GettimeStartB("LABLE");
        ...
        private void Trsftp()
        {
            Save_logfile_SFTP1(sModem, pathsaveftp);
        }
        ...
        private bool checkPingtoDUT1(string PingInformation, string EthCardName, string EthCardNameDisable, string iptest, int pingtimeout, int countPing, string mt)
        {
            bool a = false;
            bool b = true;
            string pathfile1 = Directory.GetCurrentDirectory();
            ...
            while (b)
            {
                if (!checkStatustesting(mt8)) { mt = "Checking"; b = false; }
                cmn.pingToAddress(iptest);
                msg(cmn.pingdata);
                if (!string.IsNullOrEmpty(cmn.pingdata))
                {
                    if (cmn.pingdata.Contains("Can not ping to"))
                    {
                        ...
                        if (pingtimeout1 >= pingtimeout)
                        {
                            countPing = 0;
                            pingtimeout = 0;
                            if (MyMessagesBox.Show("Đèn LED bản nháy Xanh đúng không ?") == DialogResult.Yes)
                            {
                                msg(PingInformation + " can not ping");
                                ErrorNian("Error_Reset_Issue");
                                c_pingProduct = true;
                                b = false;
                                a = false;
                                //return false;
        ...
        private bool PingIPDUTServer()
        {
            bool a = false;
            bool b = true;
            int pingtimeout1 = 0;
            int ping = 0;
            Status4("check_Ping_DUTIP_Server");
            string pathfile1 = Directory.GetCurrentDirectory();
            if (thStattus1 == true)
            {
                while (b)
                    ...
                    if (!string.IsNullOrEmpty(cmn.pingdata))
                    {
                        if (cmn.pingdata.Contains("Can not ping to"))
                        {
                            ...
                            if (pingtimeout1 >= 20)
                            {
                                ...
                                StopTime = GettimeEnd("Get_INFOR_MIB");
                                GetIFMIB = CountTime("Get_INFOR_MIB", StartTime, StopTime);
                                checkPingtoDUT1("Ping to Eth1", "Eth1", " ", InfoIP, 6, 4, mt8);
                                pingtimeout1 = 0;
                                msg("Ping DUTIP Server Fail!!");
                                Status4("Ping DUTIP Server Fail!!");
                                ErrorNian("Err_Ping_DUTIP_SERVER");
                                b = false;
                                a = false;
                                return false;
                            }
        ...
        private bool PingSreverNian()
        {
            ...
            return a;
            infomationMIBPass = true;
        }
        private bool Ping_CMIP_Nian()
        {
            bool a = false;
            bool b = true;
            int ping = 0;
            Status4("check_Ping_CMIP");
            try
            {
                if (thStattus1 == true)
                {
        ...
        private bool GetWANMAC()
        {
            ...
            if (thStattus1 == true)
            {
                ...
                string Maclan1 = khuyen.Substring(0, 4);
                string Maclan2 = khuyen.Substring(4, 4);
                string Maclan3 = khuyen.Substring(8, 4);
                string Maclan = Maclan1 + "-" + Maclan2 + "-" + Maclan3 + " ";
                        ...
                        if (telnet.Connect(inf.Ipcmts))//10.52.78.69
                        {
                            ...
                            while (c)
                            {
                                if (tel.Contains("CM has an IPv6 address"))
                                {
                                    try
                                    {
                                        CMip = tel.Substring(tel.IndexOf(Maclan), 35);
                                        CMip = CMip.Substring(CMip.IndexOf("10.52."), 14);
                                        CMip = CMip.Trim();
                                        if (CMip.Contains("10.52."))
                                        {
                                            getWANMACPass = true;
            ...
            if (thStattus1 == true)
            {
                ...
                while (b)
                {
                    ...
                    if (cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.2.1.1.1.0"))
                    {
                        ...
                        {
                            StopTime = GettimeEnd("Get_INFOR_MIB_2");
                            GetIFMIB_2 = CountTime("Get_INFOR_MIB_2", StartTime, StopTime);
                ...
                while (b)
                {
                    ...
                    if (cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.4.1.17270.50.2.2.2.1.1.3.10001"))
                    {
                        ...
                        else
                        {
                            StopTime = GettimeEnd("Get_INFOR_MIB_2");
                            GetIFMIB_2 = CountTime("Get_INFOR_MIB_2", StartTime, StopTime);
                            if (lbSsid.Text.Contains("admin") | lbSsid.Text.Contains("OBA"))
                            {
                    ...
                    {
                        cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.2.1.2.2.1.6.16");
                        msg("MTAMAC_TEST: " + cable.value);
                    ...
                    if (cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.4.1.46366.4292.80.1.3.0"))
                    {
                        ...
                        else
                        {
                            ...
                            if (va.Length == 16)
                            {
                                ...
                                {
                                    HW = "2.1";
                                    txtInfoModem.Text = "Technicolor DOCSIS 3.1 2-PORT Voice Wireless Gateway <<HW_REV: 2.1; VENDOR: Technicolor; BOOTR: S1TC-3.81.21.97; SW_REV: CGM4981COM_5.8p3s1_PROD_sey; MODEL: CGM4981COM>>";
                                }
                                else if (sModem == "CGM4981VDT")
                                {
                                    HW = "2.0";
                                ...
                                else if (sModem == "CGM4981COM-DEV")
                                {
                                    HW = "2.0";
                                    txtInfoModem.Text = "Technicolor DOCSIS 3.1 2-PORT Voice Wireless Gateway <<HW_REV: 2.0; VENDOR: Technicolor; BOOTR: S1TC-3.81.21.97; SW_REV: CGM4981COM_5.8p3s1_DEV_sey; MODEL: CGM4981COM>>";
                                }
                                else
                                {
                                    HW = "2.0";
                                    txtInfoModem.Text = "Technicolor DOCSIS 3.1 2-PORT Voice Wireless Gateway <<HW_REV: 2.0; VENDOR: Technicolor; BOOTR: S1TC-3.81.21.97; SW_REV: CGM4981COM_5.8p3s1_PROD_sey; MODEL: CGM4981COM>>";
                                }
                            }
                            else
                            {
                                if (va.Substring(6, 2).Contains("05"))
                                {
                                    ...
                                    {
                                        HW = "2.1";
                                        txtInfoModem.Text = "Technicolor DOCSIS 3.1 2-PORT Voice Wireless Gateway <<HW_REV: 2.1; VENDOR: Technicolor; BOOTR: S1TC-3.81.21.97; SW_REV: CGM4981COM_5.8p3s1_DEV_sey; MODEL: CGM4981COM>>";
                                    }
                                    else
                                    {
                                        txtInfoModem.Text = "Technicolor DOCSIS 3.1 2-PORT Voice Wireless Gateway <<HW_REV: 2.1; VENDOR: Technicolor; BOOTR: S1TC-3.81.21.97; SW_REV: CGM4981COM_5.8p3s1_PROD_sey; MODEL: CGM4981COM>>";
                                ...
                                else
                                {
                                    ...
                                    {
                                        HW = "2.0";
                                        txtInfoModem.Text = "Technicolor DOCSIS 3.1 2-PORT Voice Wireless Gateway <<HW_REV: 2.0; VENDOR: Technicolor; BOOTR: S1TC-3.81.21.97; SW_REV: CGM4981COM_4.14p6s4_DEV_sey; MODEL: CGM4981COM>>";
                                    }
                                    else
                                    {
                                        HW = "2.3";
                                        txtInfoModem.Text = "Technicolor DOCSIS 3.1 2-PORT Voice Wireless Gateway <<HW_REV: 2.3; VENDOR: Technicolor; BOOTR: S1TC-3.81.21.97; SW_REV: CGM4981COM_5.8p3s1_PROD_sey; MODEL: CGM4981COM>>";
                                    }
                                }
                            }
                        }
        ...
        private void PreCall()
        {
            ...
            try
            {
                ...
                {                 
                    msg("delete file AudioTest ");                                    
                }
                delay(500);
                if (cmn.deleteFile(Directory.GetCurrentDirectory(), "\\nsTest.mp3")) //mp3
                {                 
                    msg("delete file nsTest ");                                   
                }
                delay(500);
            }
        ...
        private bool playfile(string audioFileName)
        {
            ...
            try
            {
                ...
                {
                    delay(500);
                    voip.PlayAudio(audioFileName, 1);
                    delay(1000);
                    a = true;
                    return true;
                }
        ...
        private bool noiseTest(string bug)
        {
            ...
            bool a = false;
            VoiPNoise = false;
            if (voipf == true && Type_Program.Contains("OFF"))
            {
                return false;
            }
            if (bug == "P1toP2")
                ...
                {                 
                    ...
                    if (playfile("AudioTest.mp3"))
                    {
                        ...
                        if (voip_bool == true)
                        {
                            ...
                            msg("ResetRelay");
                            //if (!comCmd2to1("ATL2")) { return false; }
                           // delay(400);
                            //comCmd2to1_new("ATD1001");
                            if (!comCmd2to1_new("ATD1001")) { return false; }
                            else
                            {
                                voipf = true;
                                DKVOIP1 = true;
        ...
        private bool enableBLE()
        {
            ...
            {               
                ...
                {
                    if (thStattus1 == false) return false;
                    cableZB.connectionSetVer01(CMip, 1, "private", "1.3.6.1.4.1.46366.4292.10001.1.1.0", 1);
                    Delay(4500);
                    msg(cableZB.value);
                    if (cableZB.connected.Contains("1.3.6.1.4.1.46366.4292.10001.1.1.0"))
                    {
                        enableBLEPass = true;                     
                        msg("enableBLE_ok");
                        msgPass("enableBLE");            
        ...
        private bool enableZigbee()
        {
            ...
            {
                if (thStattus1 == false) return false;
                cableZB.connectionSetVer01(CMip, 1, "private", "1.3.6.1.4.1.46366.4292.10001.1.4.0", 2);
                Delay(2500);
                msg(cableZB.value);
                if (cableZB.connected.Contains("1.3.6.1.4.1.46366.4292.10001.1.4.0"))
                {
        ...
        private bool disableBLE()
        {
            ...
            int count = 0;
            // CMip = "10.52.81.237";
            cableZB.connectionSetVer01(CMip, 1, "private", "1.3.6.1.4.1.46366.4292.10001.1.1.0", 0);
            msg(cableZB.value);
            while (b)
            {
                if (mt8.Equals("Checking")) { b = false; }
        ...
        private bool disableZigbee()
        {
            ...
            {
                if (thStattus1 == false) return false;
                cableZB.connectionSetVer01(CMip, 1, "private", "1.3.6.1.4.1.46366.4292.10001.1.4.0", 1);
                msg(cableZB.value);
                Delay(5000);
                disaZigBeePass = true;
                return true;
        ...
        /// </summary>
        /// 
        public void Write_File_new_fpy(string file_path, string WriteText)
        {
            ...
            if (cvt_time_hour < 7.30 && cvt_time_hour > 0) // ca ddem
            {
                ...
                path = "D:\\LogFile_Fpy\\" + time1_1+ time6;
            }
            else
            {
                path = "D:\\LogFile_Fpy\\" + time1;
            }
            ...
            string path_all = path + "\\" + file_path + ".txt";
            try
            {
                if (File.Exists(path_all))
                {
                    File.AppendAllText(path_all, WriteText + "\r\n", Encoding.UTF8);
                }
                else
                {
                    File.WriteAllText(path_all, WriteText + "\r\n", Encoding.UTF8);
                }
        ...
        private string Read_Filetxt_fpy(string FileName)
        {
            string a = "";
            string time1 = DateTime.Now.ToString("yyyy-MM-dd");
            string path = "";
            ...
            string path_all = path + "\\" + FileName + ".txt";
            try
            {
                FileStream fs = new FileStream(path_all, FileMode.Open);
        ...
        private double Check_system_fpy(string model_name, string station)
        {
            double kq_fpyy = 100;
            string H = DateTime.Now.Hour.ToString() + "." + DateTime.Now.Minute.ToString();
            double Hour = Convert.ToDouble(H);
            string check_data = "";
            string file_check_fpy = "Loock_fpy";
            string Check_fpy_1 = "Check_fpy_1 (7.30 to 9.30)";
            string Check_fpy_2 = "Check_fpy_2 (7.30 to 9.30)";
            string Check_fpy_3 = "Check_fpy_3 (9.30 to 12.30)";
            string Check_fpy_4 = "Check_fpy_4 (12.30 to 14.30)";
            string Check_fpy_5 = "Check_fpy_5 (19.30 to 21.30)";
            string Check_fpy_6 = "Check_fpy_6 (19.30 to 21.30)";
            string Check_fpy_7 = "Check_fpy_7 (21.30 to 0.30)";
            string Check_fpy_8 = "Check_fpy_8 (0.30 to 2.30)";
            //"2024-05-25"
            string lll = DateTime.Now.ToString("dd");
            int tru = int.Parse(lll) - 1;
            int cong = int.Parse(lll) + 1;
            string Day_Tru = DateTime.Now.ToString("yyyy-MM-") + Convert.ToString(tru);
            string Day_Cong = DateTime.Now.ToString("yyyy-MM-") + Convert.ToString(cong);
            string Day_normal = DateTime.Now.ToString("yyyy-MM-dd");

            if (Hour >= 7.30 && Hour <= 9.30)
            {
                //check_data = Read_Filetxt_fpy(file_check_fpy);
                //if (!check_data.Contains(Check_fpy_1))
                //{
                //    kq_fpyy = get_system_fpy(model_name, station, Day_normal, Day_normal, "8", "9");
                //    Write_File_new_fpy(file_check_fpy, Check_fpy_1);
                //}
                //else
                //{
                msg("No_check_fpy_system");
                //}
            }
            else if (Hour >= 9.30 && Hour <= 12.30) // check tu 7h30 - 9.30
            {
                check_data = Read_Filetxt_fpy(file_check_fpy);
                if (!check_data.Contains(Check_fpy_2))
                {
                    kq_fpyy = get_system_fpy(model_name, station, Day_normal, Day_normal, "8", "9");
                    Write_File_new_fpy(file_check_fpy, Check_fpy_2);
                }
                else
                {
                    msg("No_check_fpy_system");
            ...
            else if (Hour >= 12.30 && Hour <= 14.30) // check tu 9h30 - 12.30
            {
                check_data = Read_Filetxt_fpy(file_check_fpy);
                if (!check_data.Contains(Check_fpy_3))
                {
                    kq_fpyy = get_system_fpy(model_name, station, Day_normal, Day_normal, "10", "12");
                    Write_File_new_fpy(file_check_fpy, Check_fpy_3);
                }
                else
                {
                    msg("No_check_fpy_system");
            ...
            else if (Hour >= 14.30 && Hour <= 16.30) // check tu 12h30 - 14.30
            {
                check_data = Read_Filetxt_fpy(file_check_fpy);
                if (!check_data.Contains(Check_fpy_4))
                {
                    kq_fpyy = get_system_fpy(model_name, station, Day_normal, Day_normal, "13", "14");
                    Write_File_new_fpy(file_check_fpy, Check_fpy_4);
                }
                else
                {
                    msg("No_check_fpy_system");
            ...
            else if (Hour >= 19.30 && Hour <= 21.30) 
            {
                //check_data = Read_Filetxt_fpy(file_check_fpy);
                //if (!check_data.Contains(Check_fpy_5))
                //{
                //    kq_fpyy = get_system_fpy(model_name, station, Day_normal, Day_normal, "20", "21");
                //    Write_File_new_fpy(file_check_fpy, Check_fpy_5);
                //}
                //else
                //{
                    msg("No_check_fpy_system");
            ...
            else if (Hour >= 21.30 && Hour <= 0.30) // check tu 19h30 - 21.30
            {
                check_data = Read_Filetxt_fpy(file_check_fpy);
                if (!check_data.Contains(Check_fpy_6))
                {
                    kq_fpyy = get_system_fpy(model_name, station, Day_normal, Day_normal, "20", "21");
                    Write_File_new_fpy(file_check_fpy, Check_fpy_6);
                }
                else
                {
                    msg("No_check_fpy_system");
            ...
            else if (Hour >= 0.30 && Hour <= 2.30) // check tu 21h30 - 0.30
            {
                check_data = Read_Filetxt_fpy(file_check_fpy);
                if (!check_data.Contains(Check_fpy_7))
                {
                    kq_fpyy = get_system_fpy(model_name, station, Day_Tru, Day_normal, "22", "00");
                    Write_File_new_fpy(file_check_fpy, Check_fpy_7);
                }
                else
                {
                    msg("No_check_fpy_system");
            ...
            else if (Hour >= 2.30 && Hour <= 4.30) // check tu 0h30 - 2.30
            {
                check_data = Read_Filetxt_fpy(file_check_fpy);
                if (!check_data.Contains(Check_fpy_8))
                {
                    kq_fpyy = get_system_fpy(model_name, station, Day_normal, Day_normal, "1", "2");
                    Write_File_new_fpy(file_check_fpy, Check_fpy_8);
                }
                else
                {
                    msg("No_check_fpy_system");
        ...
        private double get_system_fpy(string model_name, string station,string S_date_time, string E_date_time, string s_time, string E_time)
        {
            msg("**************************Check FPY**************************");
            double FPY = 100;
            string a = model_name + " " + station + " " + S_date_time + " " + E_date_time + " " + s_time + " " + E_time;
            cmd.CmdConnect();
            cmd.CmdWrite("Requests_FPY.exe " + '"' + a + '"');
            Delay(500);
            cmd.CmdWrite("exit");
            string Output = cmd.CmdRead();
            msg(Output);
            string[] kk = Output.Split('\n');
            foreach (string n in kk)
            {
                if (n.Contains("FPY="))
                {
                    string data_FPY = n.Replace(" ", "").Replace("\n", "").Replace("\r", "").Replace("FPY=", "").Trim();
                    FPY = Convert.ToDouble(data_FPY);
                    break;
                }
            }
            //if (FPY < 97)
        ...
        private static Timer _timeoutTimerr;
        private static string _caption;

        public static void Show(string text, string caption, int timeout)
        {
            _caption = caption;
            _timeoutTimerr = new Timer { Interval = timeout};
            _timeoutTimerr.Tick += OnTimerTick;
            _timeoutTimerr.Start();
            //MessageBox.Show(text, caption, MessageBoxButtons.OKCancel);
          
            //MyMessagesBox.Show(text);
        }
        ...
        private static Timer _timeoutTimerr2;
        private static string _caption2;

        public static void Show2(string text, string caption, int timeout)
        {
            _caption2 = caption;
            _timeoutTimerr2 = new Timer { Interval = timeout };
            _timeoutTimerr2.Tick += OnTimerTick2;
            _timeoutTimerr2.Start();
            //MessageBox.Show(text, caption, MessageBoxButtons.OKCancel);

            //MyMessagesBox.Show(text);
        }

        private static void OnTimerTick2(object sender, EventArgs e)
        {
            _timeoutTimerr2.Stop();
            try
            {
                IntPtr mbWnd2 = FindWindow2(null, _caption2);
                SendMessage(mbWnd2, WM_CLOSE2, IntPtr.Zero, IntPtr.Zero);
            }
            catch { }
        ...
        [DllImport("user32.dll", SetLastError = true)]
        private static extern IntPtr FindWindow2(string lpClassName, string lpWindowName);

        [DllImport("user32.dll", CharSet = CharSet.Auto)]
        private static extern int SendMessage2(IntPtr hWnd, uint Msg, IntPtr wParam, IntPtr lParam);
        private const uint WM_CLOSE2 = 0x0010;
        //

        string IP_SFC_FQA1_1 = "";
        string IP_SFC_FQA2 = "";
        public DialogResult kq_stust { get; set; }
        private void Form1_Load(object sender, EventArgs e)
        {
            //Show("", "Xac nhan/Confirm", 5000);
            //kq_stust = MyMessagesBox_new1.Show("\n\n              Rut_RF_CABLE!!");
            ...
            //}
            //delay(5000);
            //Show2("", "Xac nhan/Confirm", 5000);
            //kq_stust = MyMessagesBox_new2.Show("\n\n           Cam_RF_MOCA!!");
            //if (kq_stust != DialogResult.Yes && kq_stust != DialogResult.No)
            //{
            //    msg("");
            //}
            ...
            //Write_File_new_fpy("Loock_fpy", "ok");
            //Read_Filetxt_fpy("Loock_fpy");
            //double a = Check_system_fpy("CGM4981ROG", "T05XB8OBA001");
            //SNsample.txt
            //file_result_test_pre.txt
            //ERROR_LOCK.txt

            lbl_debug_P.Visible = false;
            lbl_debug_F.Visible = false;
            lbl_pass_debug.Visible = false;
            lbl_fail_debug.Visible = false;
            btn_unlock.Visible = false;
            try
            {
                string file_path = Application.StartupPath + "\\Config_Templete.ini";
                StreamReader loadinfor = new StreamReader(file_path, Encoding.UTF8);
                string Info1 = loadinfor.ReadToEnd();
                loadinfor.Close();
                string[] nn = Info1.Split('\n');
                foreach (string n in nn)
                {
                    if (n.Contains("IP_SFC_FQA1_1"))
                    {
                        IP_SFC_FQA1_1 = n.Replace("IP_SFC_FQA1_1", "").Replace(":", "").Replace(" ", "").Replace("\r", "");
                    }
                    else if (n.Contains("IP_SFC_FQA2"))
                    {
                        IP_SFC_FQA2 = n.Replace("IP_SFC_FQA2", "").Replace(":", "").Replace(" ", "").Replace("\r", "");
                    }
                }

            }
            catch { msg("Chua co file Config_Templete.ini"); }
        ...
        private bool checkDuplicateTest(string DataSNcheck)
        {
            string data = "";
            string Path1 = Directory.GetCurrentDirectory() + "\\SNtest.txt";
            bool a = true;    
            ...
            try
            {
                ...
                loadledGreen.Close();
                if (data.Contains(DataSNcheck))
                {
                    var dupsn = data.Split('\n');
                    if (dupsn.Length >= 100) { File.Delete(Directory.GetCurrentDirectory() + "\\SNtest.txt"); }
                    return false;
                }
                else
                {
        ...
        private bool checkDup_sample(string DataSNcheck)
        {
            string data = "";
            string Path1 = Directory.GetCurrentDirectory() + "\\SNsample.txt";
            bool a = false;
            //List<string> dataSN = new List<string>();
            //dataSN = cmn.loadFileNian("SNtest.txt");         
            try
            {
                ...
                data = loadledGreen.ReadToEnd();
                loadledGreen.Close();
                if (DataSNcheck.Contains(data.Replace(" ","").Replace("\r", "").Replace("\n", "").Trim()))
                {
                    return true;
                }
                else
        ...
        private void khoitao()
        {
            ...
            valueGreen_WPS = 0.00;
            valueBlue_WPS = 0.00;
            Time_Unplus_cable_RF = 0.00;
            Time_plus_cable_MOCA = 0.00;
            valueX_WPS = 0.00;
            valueY_WPS = 0.00;
            TimepingEth1 = 0.0;
            TimepingEth2 = 0.0;
            ...
            TimeMoca = 0.0;
            TimeVoip1 = 0.0;
            StartTimeL_online = 0.00;
            StartTimeL_USB_C = 0.00;
            StartTimeL_WPS = 0.00;
            StartTimeV_voip_1 = 0.00;
            StartTimeV_noise_1 = 0.00;
            StartTimeV_voip_2 = 0.00;
            StartTimeV_noise_2 = 0.00;
        ...
        private void testing()
        {
            timeTest.Enabled = true;
            WriteC = false; 
            if (clickbootup == false)
            {
                WriteC = true;
                MyMessagesBox.Show("\n\n Pls. Click TIME_DUT_BOOTUP");
            }
            clickbootup = false;
            khoitao();
            Clear_SFC();
            Clear_failtxt();
            Failtxt.Visible = false;
            textBox1.Clear();
            VoipAgain = true;
            countTimeTest = 0;
            lbTimeShow.Focus();
            ...
            try
            {
                ...
                StartTimeT = GettimeStart("TotalTimeTest");
                StartTime = GettimeStart("INITIAL_SETUP");
                ...
                if (Label_Test == true)
                {
                    ...
                    for (int d = 0; d <= 300; d++)
                    {
                        ...
                        if (d > 200) 
                        {
                            StopTime = GettimeEnd("INITIAL_SETUP");
                            Setup_first = CountTime("INITIAL_SETUP", StartTime, StopTime);
                            ErrorNian("Error_Get_SN"); 
                        }
                    }
                }
                if (WriteC == true)
                {
                    LogFilePassNianTetst("Don't click TIME_DUT_BOOTUP");
                }
                else
                {
                    LogFilePassNianTetst("");
                }
                if (cmmType.SelectedIndex == 1) 
                {
                    if (Check_lock_may.Text == "Lock")
                    {
                        if (!checkDup_sample(lbsndut.Text.Trim()))
                        {
                            StopTime = GettimeEnd("INITIAL_SETUP");
                            Setup_first = CountTime("INITIAL_SETUP", StartTime, StopTime);
                            MsBox.Show("Yêu cầu phải test với sample để mở khóa");
                ...
                {
                    if (!checkDuplicateTest(lbsndut.Text.Trim()))
                    {
                        StopTime = GettimeEnd("INITIAL_SETUP");
                        Setup_first = CountTime("INITIAL_SETUP", StartTime, StopTime);
                        ErrorNian("Err_Dupble_SN--> ID002"); return;
                    ...
                    else
                    {
                        ...
                        if (!Check_SN(lbsndut.Text.Trim()))
                        {
                            StopTime = GettimeEnd("INITIAL_SETUP");
                            Setup_first = CountTime("INITIAL_SETUP", StartTime, StopTime);
                            StatusERR("Check_SN"); ErrorNian("Err_Check_SN_SFC"); return;
                        }
                        try
                        {
                            Thread_SFC_new.Abort();
                ...
                if (LedPhone.Contains("yes"))
                {
                    ...
                    for (int d = 0; d <= 300; d++)
                    {
                        ...
                        if (d > 200) 
                        {
                            StopTime = GettimeEnd("ETH1");
                            TimepingEth1 = CountTime("ETH1", StartTime, StopTime);
                            ErrorNian("LED_Signal_VOIP"); 
                        }
                    }
                }
                if (!checkPingEth1_new(IP_ETH1))
                {  
                    return;
                }
                StopTime = GettimeEnd("ETH1");
                ...
                StartTime = GettimeStart("Get_INFOR_MIB");
                ...
                if (!PingIPDUTServer())
                {
                    //StopTime = GettimeEnd("Get_INFOR_MIB");
                    //GetIFMIB = CountTime("Get_INFOR_MIB", StartTime, StopTime);
                    StatusERR("Err_Ping_DUTIP_SERVER"); return;
                }
                cmn.openExe1(pathfile1, "disableEthAll");
                delay(4000);
                StopTime = GettimeEnd("Get_INFOR_MIB");
                ...
                //        StopTime = GettimeEnd("Get_INFOR_MIB");
                //        GetIFMIB = CountTime("Get_INFOR_MIB", StartTime, StopTime);
                //        StatusERR("checkInformation"); return;
                //    //}
                //}
                ////delay(1000);
                //if (!getCMMAC())
                ...
                //        StopTime = GettimeEnd("Get_INFOR_MIB");
                //        GetIFMIB = CountTime("Get_INFOR_MIB", StartTime, StopTime);
                //        StatusERR("getMTAMAC"); return;
                //    //}
                //}
                ////delay(1000);
                //getMACBLE();
                //bool getSSIDName_lan11 = false;
                //if (!getSSIDName_lan1())
                //{
                //    msg("GetSSIDName_lan1_fail");
                //    getSSIDName_lan11 = true;
                //}
                ////delay(1000);
                //if (!getWifiPassword())
                //{
                ...
                //        StopTime = GettimeEnd("Get_INFOR_MIB");
                //        GetIFMIB = CountTime("Get_INFOR_MIB", StartTime, StopTime);
                //        StatusERR("GetPassword"); return;
                //    //}
                //}
                //if (getSSIDName_lan11 == true)
                //{
                //    if (!getSSIDName())
                //    {
                //        //if (Type_Program == "ON")
                ...
                StartTime = GettimeStart("ETH3");
                if (!checkPingEth3_new())
                {
                    //if (Type_Program == "ON")
                    //{
                        //StopTime = GettimeEnd("ETH3");
                ...
                StartTime = GettimeStart("ETH4");
                if (!checkPingEth4_new())
                {
                    //if (Type_Program == "ON")
                    //{
                        //StopTime = GettimeEnd("ETH4");
                ...
                StartTimeS = GettimeStart("SPEED");
                if (!Check_speed_new("All", "1000"))
                {
                    //if (Type_Program == "ON")
                    //{
                        //StopTimeS = GettimeEnd("SPEED");
                        //TimeSpeed1G = CountTime("SPEED", StartTimeS, StopTimeS);
                        return;
                    //}
                }
                if (sModem == "CGM4981COM" | sModem == "CGM4981COX" | sModem == "CGM4981SHW" | sModem == "CGM4981ROG"| sModem == "CGM4981COM-DEV")
                ...
                for (int d = 0; d <= 500; d++)
                {
                    ...
                    else if (LockloginWeb == false && dkresetodefoult == false && CheckLed_OnlineFail == true && dkvoipretest == false /*&& disaZigBeeFail == true*/) break;
                    else
                    {
                        if (thStattus1 == false) return;
                ...
                // 
                //*************************************************************************** len suwa
                StartTime = GettimeStart("Get_INFOR_MIB_2");
                cmn.openExe1(pathfile1, "enableEthSever");
                delay(2000);
                cmn.openExe1(pathfile1, "disableEthAll");
                delay(5000);
                ...
                StartTime = GettimeStart("GET_INFOR_WEB");
                cmn.openExe1(pathfile1, "disableEthSever");
                if (!WebAutoNian())
                {
                    //if (Type_Program == "ON")
                ...
                {
                    accessWebsiteNewPassCon = true;
                }
                StopTime = GettimeEnd("GET_INFOR_WEB");
                Timeweb = CountTime("GET_INFOR_WEB", StartTime, StopTime);

                StartTime = GettimeStart("MOCA");
                if (!enableMocaWeb())
                {
                    //if (Type_Program == "ON")
                    //{
                        //StopTime = GettimeEnd("MOCA");
                        //TimeMoca = CountTime("MOCA", StartTime, StopTime);
                        StatusERR("noiseTest"); return;
                    //}
                }
                if (!checkPingMocaMian())
                {
                    //if (Type_Program == "ON")
                    //{
                        StopTime = GettimeEnd("MOCA");
                        TimeMoca = CountTime("MOCA", StartTime, StopTime);
                        StatusERR("checkPingMoca"); return;
                    //}
                    //else
                    //{
                    //    msgFail("MOCA =========> FAIL");
                ...
                StartTime = GettimeStart("RESET");
                textBox1.Clear();
                c_pingProduct = true;
                if (!checkPingResetTodefault())
                {
                    //if (Type_Program == "ON")
                    //{
                        //StopTime = GettimeEnd("RESET");
                        //FactoryRST = CountTime("RESET", StartTime, StopTime);
                        StatusERR("Reset_To_Default"); return;
                   // }
                }
                //StopTime = GettimeEnd("RESET");
                //FactoryRST = CountTime("RESET", StartTime, StopTime);
                ...
                //StopTimeT = GettimeEnd("TotalTimeTest");
                //TotalTest_Time = CountTime1("TotalTimeTest", StartTimeT, StopTimeT);
                //Save_Time_test_all(sModem, SNlabel, DUTresult, ERR_CODE, TotalTest_Time);
            }
            catch
            {
        ...
        private bool GetWanMac()
        {
            ...
            if (thStattus1 == true)
            {
                ...
                {
                    if (thStattus1 == false) return false;
                    if (cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.2.1.2.2.1.6.1"))
                    {
                        //delay(c);
                        string va = cable.value;
                        va = va.Replace("\r\n", "");
        ...
        private void timeTest_Tick(object sender, EventArgs e)
        {
            ...
            //else
            //{
            //    if (countTimeTest >= 1200)
            //    {
            //        Status4("Error Time Test 1200 seconds");
            //        msg("Error Time Test 1200 seconds ");
            //        fail("Error Time Test 1200 seconds!");
            //        timStart.Enabled = false;
            //        timeTest.Enabled = false;
            //        Status("Stop Program");
            //        return;
        ...
        private void AnalysisVoip(string key)////timVoip_Tick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (value.Count >= 150)  //190
                {
                    ...
                    {
                        string key1 = String.Join(Environment.NewLine, value.ToArray());
                        msg(key1);
                        noise = false;
                        voipNoiseResult = false;
                        noisevalue1 = 0;
                    ...
                    else
                    {
                        ...
                        analysDup = voip.AnalysDup(value);
                        analysDup.Sort();
                        msg(String.Join(Environment.NewLine, analysDup.ToArray()));
                        //foreach (double an in analysDup)
                        //{
                        //    //if (p11 == true)
                        //    //{
        ...
        private void Form1_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            else
            {
                ...
                if (thStattus1 == true)
                {
                    LogFilePassNianTetst("Close_Program");
                    LogFile("FAIL", "Close_Program.", "LOOSE");
                }
                cmn.kill("XLAXB8");
                cmn.kill("cmd");
                cmn.kill("msedge");
           ...
           while(txtComFB.Text.Contains("Dong") | txtComFB.Text.Contains("CLOSE_OK") & timS == true)
            {
                ...
                int cvt_time_hour = int.Parse(time_hour);

                ma_loi = read_result_test_pre();
                if (ma_loi != "" & Unlocked == false)
                {
                    if (!cmmType.Text.Contains("Offline_Program"))
                    {
                        cmmType.Text = "Offline_Program";
                        Log.Save_Logs_File_status_program("Locked", ma_loi, Login_pass.acc);
                        if (ma_loi.Contains("FPY"))
                        {
                            MsBox.Show("Chương trình đã bị khóa do FPY thấp\n Vui lòng liên hệ kỹ sư đến xử lý vấn đề");
                        }
                        else
                        {
                            MsBox.Show("Chương trình đã bị khóa\n Vui lòng liên hệ kỹ sư đến xử lý vấn đề\nMã lỗi: " + ma_loi);
                        }
        ...
        /// <returns></returns>
        /// 
        private bool check_debug()
        {
            bool a = false;
            if (int.Parse(lbl_pass_debug.Text) >= 1)
            {
                p_debug = 0;
                f_debug = 0;
                if (txtOutput.InvokeRequired) { txtOutput.Invoke(new Action(() => { txtOutput.Text = "0"; })); }
                else { txtOutput.Text = "0"; }
                try
                {
                    string path = Application.StartupPath + "\\" + "file_result_test_pre.txt";
                    File.Delete(path);
                }
                catch { }
                try
        ...
        private void Fail_debug(int Message)
        {
            if (lbl_pass_debug.InvokeRequired)
            {
                lbl_pass_debug.Invoke(new Action(() => lbl_pass_debug.Text = Message.ToString()));
            }
            else
            {
                lbl_pass_debug.Text = Message.ToString();
            }
        }
        private string read_result_test_pre()
        {
            string result_pre = "";
            string path = "ERROR_LOCK.txt";
            if (!File.Exists(path))
            {
                //result_pre = "PASS";
            }
        ...
        private string read_file(string FileName)
        {
            ...
            try
            {
                string partSignalstart = FileName;
        ...
        private bool write_log_ERR_lock(string ERR)
        {
            bool a = false;
            try
            {
                string key = read_file("file_result_test_pre.txt");
                ERR_CODE1 = key;
                if (key.Contains(ERR))
                {
                    ...
                    write_result_test_pre("ERROR_LOCK.txt", ERR);
                    write_result_test_pre("file_result_test_pre.txt", ERR + "\n");
                    return false;
                }
                else 
                {
        ...
        public void write_result_test_pre(string file_path, string WriteText)
        {
            try
            {
                if (File.Exists(file_path))
                {
                    File.AppendAllText(file_path, WriteText, Encoding.UTF8);
                }
                else
                {
                    File.WriteAllText(file_path, WriteText, Encoding.UTF8);
                }
            }
            catch (Exception u)
            {
            ...
            if (thStattus1 == true)
            {
                ...
                for (int c = 1; c <= 8; c++)
                {
                    try
                    {
                        ...
                        string va = "";
                        va = ((Gecko.GeckoHtmlElement)(geckoWebBrowser1.Document.DocumentElement)).InnerHtml.ToString();
                        msg("Get web Black: " + va);
                        if (WaitLog(DocumentTitleWEBSSID2, DocumentTitleWEBSSID2))
                        {
                            StopTime = GettimeEnd("GET_INFOR_WEB");
                            Timeweb = CountTime("GET_INFOR_WEB", StartTime, StopTime);
                            MessageBox.Show("Sản phẩm không trang đen\r\nGọi kỹ sư kiểm tra");
        ...
        private bool enableMocaWeb()
        {
            ...
            if (thStattus1 == true && accessWebsiteNewPassCon == true)
            {
                        ...
                        while (true)
                        {
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    if (c >= 2)
                                    {
                                        StartTime = GettimeStart("Plug_MOCA");
                                        //timeTest.Enabled = false;
                                        Show2("", "Xac nhan/Confirm", 20000);
                                        kq_stust = MyMessagesBox_new2.Show("\n\n           Cam_RF_MOCA!!");
                                        if (kq_stust != DialogResult.Yes && kq_stust != DialogResult.No)
                                        {
                                            msg("OP time out plug RF cable MOCA");
                                            StopTime = GettimeEnd("Plug_MOCA");
                                            Time_plus_cable_MOCA = CountTime("Plug_MOCA", StartTime, StopTime);
                                            StopTime = GettimeEnd("MOCA");
                                            TimeMoca = CountTime("MOCA", StartTime, StopTime);
                                            ErrorNian("Err_timeout_OP_plug_RF_cabel"); return false;
                                        }
        ...
        private void enableTelnetToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            TimeVoip2 = CountTime("VOIP2", TimeStart1, TimeEnd1);
            Status4("CHECK VOIP COMPLETE");
        }
        //******************************************************************************************
        //******************************************************************************************
        ...
        bool time_out_terminal = false;
        public bool Terminal_SFC_new()
        {
            time_out_terminal = false;
            kq_Terminal_SFC = false;
            bool a = false;
            string str = "";
            while (true)
            {
                try
                {
                    ...
                    string IPSFC = "";
                    string tram = "";
                    if (checkbox_FQA2.Checked == false)
                    {
                        IPSFC = IP_SFC_FQA1_1;
                        tram = "FQA1_1";
                    }
                    ...
                    while (true)
                    {
                        ...
                        //TcpClient clinr = listener_SFC.AcceptTcpClient();
                        //Listconnect.Add(clinr);
                        socket2 = socket_SFC.RemoteEndPoint.ToString();  //get ip connection 10.228.110.198 // FQA1_1

                        if (socket2.Contains(IPSFC))
                        {
                            msg_SFC(V.ToString());
                            msg_SFC("IP: " + socket2);
                            break;
                        ...
                        {
                            if (Math.Round((DateTime.Now.Ticks - cout_dem) / 10000000, 2) > 60)
                            {
                                time_out_terminal = true;
                                return false;
                            }
                        }
                    }
                    ...
                    msg_SFC(str);
                    ////////////////////// 
                    if (str.Contains("@_@") && str.Contains(tram))
                    {
                        kq_Terminal_SFC = true;
                        //stream_SFC.Close();
                        //socket_SFC.Close();
                    ...
                    if (str.Contains("@_@") && str.Contains("FQA1_1"))
                    {
                        ...
                        while (true)
                        {
                            ...
                            catch
                            {
                                ...
                                //break;
                                Clear_SFC();
                                IPAddress address1 = IPAddress.Parse("0.0.0.0");
                                listener_SFC = new TcpListener(address, 55962);
                                // 1. listen
                                listener_SFC.Start();
                                socket_SFC = listener_SFC.AcceptSocket();
                                ...
                                writer_SFC = new StreamWriter(stream_SFC);
                                writer_SFC.AutoFlush = true;
                                byte[] data1 = encoding.GetBytes("config");
                                stream_SFC.Write(data, 0, data.Length);
                                Write_logfile_SFC("Send: config");
                                Delay(500);
                                // 2. receive
        ...
        private void send_data_SN_SFC(string data)
        {
            try
            {
                writer_SFC.Write("\x1b\x07" + data + "\r\n");
                Write_logfile_SFC("Send: " + "\x1b\x07" + data + "\r\n");
                msg("Send: " + "\x1b\x07" + data + "\r\n");
                Delay(500);
            }
            catch { }
        }
        ...
        private void send_data_SFC_FAIL(string data)
        {
            try
            {
                writer_SFC.Write("\x1b\x07" + data + "\r\n");
        ...
        private void Clear_failtxt()
        {
            if (Failtxt.InvokeRequired)
            {
                Failtxt.Invoke(new Action(() => Failtxt.Clear()));
            }
            else Failtxt.Clear();
        }
        ...
        private bool Check_SN(string SN_Box)
        {
            ...
            for (int i = 0; i <= 1; i++)
            {
                string get_sfc = "";
                string get_sfc2 = "";
                send_data_SN_SFC(SN_SFC);
                get_sfc = Get_data_SFC();
                get_sfc2 = Get_data_SFC2();

                if (txtSN_SFC.InvokeRequired) 
                {
                    txtSN_SFC.Invoke(new Action(() => { txtSN_SFC.Text = SN_Box; })); 
                }
                else{ txtSN_SFC.Text = SN_Box; }

                if ((get_sfc.Contains("PASS") || get_sfc2.Contains("PASS")) && (get_sfc2.Contains(SN_Box) || get_sfc.Contains(SN_Box))) //|| (get_sfc2.Contains("PASS") && get_sfc2.Contains(SN_Box)))
                {
                    if (lblSFC_Status.InvokeRequired)
                    {
                        lblSFC_Status.Invoke(new Action(() => { lblSFC_Status.Text = "Send data to SFC: Success"; }));
                    }
                    else { lblSFC_Status.Text = "Send data to SFC: Success"; }
                ...
                else
                {
                    if (i >= 1)
                    {
                        ...
                        else { lblSFC_Status.Text = "Send data to SFC: Unsuccess"; }
        ...
        private bool Check_Feedback(string chuoi)
        {
            try
            {
                ...
                if ((temp.Contains("PASS")) | (temp.Contains("ERRO")))
                {
                    return true;
                }
                else
        ...
        private bool Send_To_SFC_PASS()
        {
            ...
            if (cmmType.SelectedIndex == 1)
            {
                ...
                {
                    p_debug++;
                    if (lbl_pass_debug.InvokeRequired) { lbl_pass_debug.Invoke(new Action(() => { lbl_pass_debug.Text = p_debug.ToString(); })); }
                    else { lbl_pass_debug.Text = p_debug.ToString(); }
                    if (check_debug())
                    {
                        lbl_debug_P.Visible = false;
                        lbl_debug_F.Visible = false;
                        ...
                        MsBox.Show("Chương trình đã mở khóa thành công\r\nThank you very much!!");
                        cmmType.Text = "Online_Program";
                    }
                }
                return true;
            ...
            else
            {
                ...
                send_data_SFC_PASS(STB_SN + "GETWIFIBYWEB" + Computer_name + "\r");
                string hi = Get_data_SFC();
                string hi2 = Get_data_SFC2();
                ...
                if (hi.Contains("PASS") || hi2.Contains("PASS")) //if (Check_Feedback(str_SFC))
                {
        ...
        private bool Send_To_SFC_FAIL(string err)
        {
            ...
            send_data_SFC_FAIL(STB_SN + "FUNCTIONFAIL" + Computer_name + err + "\r");
            string hi = Get_data_SFC();
            string hi2 = Get_data_SFC2();

            if (hi.Contains("PASS") || hi2.Contains("PASS") || hi.Contains("ERRO") || hi2.Contains("ERRO")) //Check_Feedback(buf)
        ...
        private void Check_ERRCODEoff(string err)
        {
            ///maloi    
            ///
            ERR_CODE = "";
            ...
            if (err == "Err_timeout_OP_unplug_RF_cabel") ERR_CODE = "EOPURF"; 
            if (err == "Err_timeout_OP_plug_RF_cabel") ERR_CODE = "EOPPRF";

            if (err == "Err_Ping_after_unplug_RF_cabel") ERR_CODE = "FPAFRF";
        ...
        private void Check_ERRCODE(string err)
        {
            ...
            if (Type_Program == "ON")
            {
                write_log_ERR_lock(ERR_CODE + " - " + err);
        ...
        private bool Check_speed(string namecheck, string speed)
        {
            ...
            //double TimeStart = 0;
            if (speed.Contains("1000"))
            {
                 //TimeStart = GettimeStart("Speed1G_Eth1,2,3 - 2.5G_Eth4");
            }
            //else
            //{
            //    //TimeStart = GettimeStart("Speed100M_All");
            //}
            ////////////////////////////////////
            if (thStattus1 == true)
            {
                while (b)
                {
                    ...
                    {                     
                        ...
                        if (speed_number == 5500)
                        {
                            msgPass("checkSpeed 1G & 2.5G/Eth4");                          
                            speed_number = 0;
                    ...
                    {                     
                        ...
                        if (speed_number == 400)
                        {
                            ...
                            //TimeSpeed100M = CountTime("Speed100M", TimeStart, TimeEnd);
                            return true;
                        }
                        else
                        {
        ...
        private bool Check_speed_new(string namecheck, string speed)
        {
            Status("Speed " + speed);
            string pathfile1 = Directory.GetCurrentDirectory();
            Status4("check Speed:" + speed + "/" + namecheck);
            ...
            if (thStattus1 == true)
            {
                ...
                {
                    string speed_number = "";
                    speed_number = cmn.xb7speedEthAll_string(namecheck, speed);
                    msg("Speed = " + speed_number);
                    if (speed == "1000")
                    {
                        msg("Checking Speed1G:   " + count.ToString());
                        if (speed_number.Contains("PASS"))
                        {
                            msgPass("checkSpeed 1G & 2.5G/Eth4");
                            checkSpeed1GPass = true;
                            b = false;
                        ...
                        else
                        {
                            ...
                            {
                                DKlED1G = true;
                                string[] data = speed_number.Split('\n');
                                foreach (string i in data)
                                {
                                    if (i.Contains("Eth1"))
                                    {
                                        speed_eth1 = i.Replace("Eth1:", "").Replace(" ", "").Trim();
                                    }
                                    else if (i.Contains("Eth2"))
                                    {
                                        speed_eth2 = i.Replace("Eth2:", "").Replace(" ", "").Trim();
                                    }
                                    else if (i.Contains("Eth3"))
                                    {
                                        speed_eth3 = i.Replace("Eth3:", "").Replace(" ", "").Trim();
                                    }
                                    else if (i.Contains("Eth4"))
                                    {
                                        speed_eth4 = i.Replace("Eth4:", "").Replace(" ", "").Trim();
                                    }
                                }
                                StopTimeS = GettimeEnd("SPEED");
                                TimeSpeed1G = CountTime("SPEED", StartTimeS, StopTimeS);
                                if (!speed_eth1.Contains(speed))
                                {
                                    ErrorNian("Err_Eth1_Speed1G");
                                }
                                else if (!speed_eth2.Contains(speed))
                                {
                                    ErrorNian("Err_Eth2_Speed1G");
                                }
                                else if (!speed_eth3.Contains(speed))
                                {
                                    ErrorNian("Err_Eth3_Speed1G");
                                }
                                else if (!speed_eth4.Contains("2500"))
                                {
                                    ErrorNian("Err_Eth4_Speed2.5G");
                                }
                                b = false;
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (count100 >= 3)
                            {
                                ...
                                else if (!speed_eth4.Contains(speed))
                                {
                                    ErrorNian("Err_Eth4_Speed100M");
                                }
        ...
        public string GetKeyIDEth(string databuffer)  //chi su dung ham nay khi chuoi do la duy nhat tren databuffer
        {
            string result = "";
            if (!string.IsNullOrEmpty(databuffer))
            {
                result = databuffer.Substring(5, 4);
            }
            return result;
        }
        ...
        string pronam = "CGM4981COM";//CGM4981COM_5.8p3s1_PROD_sey; 
        string inforSW = "Technicolor DOCSIS 3.1 2-PORT Voice Wireless Gateway <<HW_REV: 2.3; VENDOR: Technicolor; BOOTR: S1TC-3.81.21.97; SW_REV: CGM4981COM_4.14p6s4_PROD_sey; MODEL: CGM4981COM>>";
        private void cmmModem_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            if (cmmModem.SelectedIndex == 0| cmmModem.SelectedIndex == -1)
            {
                ...
                HW = "2.3";
                model = "1";
                inforSW = "Technicolor DOCSIS 3.1 2-PORT Voice Wireless Gateway <<HW_REV: 2.3; VENDOR: Technicolor; BOOTR: S1TC-3.81.21.97; SW_REV: CGM4981COM_5.8p3s1_PROD_sey; MODEL: CGM4981COM>>";
            ...
            if (cmmModem.SelectedIndex == 1)
            {
                ...
                cmmModem.Enabled = false;
                WriteFileLabView("MODEL", "1");
                inforSW = "Technicolor DOCSIS 3.1 2-PORT Voice Wireless Gateway <<HW_REV: 2.1; VENDOR: Technicolor; BOOTR: S1TC-3.81.21.97; SW_REV: CGM4981COM_5.8p3s1_DEV_sey; MODEL: CGM4981COM>>";
                label9.Text = "Ver: 1.4";
            }
            ...
            if (cmmModem.SelectedIndex == 3)
            {
                ...
                DocumentTitleChangPassword = "Change Password";
                DocumentTitleInfor = "Gateway > Connection";
                inforSW = "Technicolor DOCSIS 3.1 2-PORT Voice Wireless Gateway <<HW_REV: 2.1; VENDOR: Technicolor; BOOTR: S1TC-3.81.21.97; SW_REV: CGM4981COM_5.8p3s1_PROD_sey; MODEL: CGM4981COM>>";
            }        
        ...
        private bool countmaintain()
        {
            ...
            try
            {
                ...
                loadinfor2.Close();
                msg("Count Maintance Equipment: \n" + Datacount);
                if (Timee3 < Timee2 & Timee3 > Timee1)
                {
                    if (RF1 > 3069 | RF2 > 3069)
                    {
                        MyMessagesBox.Show("\n                  WARNING!!! \n Pls.Gọi kỹ sư Online QE sửa chưa \n                    RF cable");
                        if (RF1 > 3384 | RF2 > 3384)
                        {
                            return false;
                        }
                    }
                    if (Eth1 > 4437 | Eth2 > 4437 | Eth3 > 4437 | Eth4 > 4437)
                    {
                        MyMessagesBox.Show("\n                  WARNING!!! \n Pls.Gọi kỹ sư Online QE sửa chưa \n                    RJ45 cable");
                        if (Eth1 > 4752 | Eth2 > 4752 | Eth3 > 4752 | Eth4 > 4752)
                        {
                            return false;
                        }
                    }
                    if (PSU1 > 4800 | PSU2 > 4800)
                    {
                        MyMessagesBox.Show("\n                  WARNING!!! \n Pls.Gọi kỹ sư Online QE sửa chưa \n                        PSU");
                    ...
                    if (VOIP1 > 4437 | VOIP2 > 4437)
                    {
                        MyMessagesBox.Show("\n                  WARNING!!! \n Pls.Gọi kỹ sư Online QE sửa chưa \n                    RJ11 Cable");
                        if (VOIP1 > 4752 | VOIP2 > 4752)
                        {
                            return false;
                        }
                    }
        ...
        private bool Geterroff(string keya)
        {
            bool a = false;
            string pathfilea = Directory.GetCurrentDirectory() + "\\Offline.txt";
            StreamReader loadinfora = new StreamReader(pathfilea, Encoding.UTF8);
            keya = loadinfora.ReadToEnd().Trim();
            loadinfora.Close();
            if (keya.Contains("PASS"))
            {
                a = true;
            }
            else
        ...
        private void checkstatus()
        {
            try
            {
                ...
                loadinfor.Close();
                string pathfile2 = Directory.GetCurrentDirectory() + "\\Statuslockduperr.txt";
                StreamReader loadinfor1 = new StreamReader(pathfile2, Encoding.UTF8);
                string status1 = loadinfor1.ReadToEnd();
                loadinfor1.Close();
                if (status.Contains("lock") | status1.Contains("lock"))
                {
                    if (!Geterroff("PASS"))
                    {
                        if (cmmType.InvokeRequired)
            ...
            foreach (NAudio.CoreAudioApi.MMDevice dev in devcol)
            {
                try
                {
                    if(dev.State == NAudio.CoreAudioApi.DeviceState.Active && (dev.FriendlyName.ToLower().Contains("micro")))
                    {
                        ...
                        msg("Adjust_volume_Microphone_100%");
                    }
                }
                catch {
        ...
        private bool EthnetCardCheck()
        {
            ...
            for (int i = 1; i <= 2; i++)
            {
                ...
                Output = "";
                cmd.CmdConnect();
                cmd.CmdWrite("wmic nic get Caption,NetConnectionID");
                Delay(200);
                Output = cmd.CmdRead();
                cmd.CmdWrite("exit");
                ...
                if (Eth1 == false)
                {
                    if (Output.Contains("Eth1")) { Eth1 = true; }
                    else
                    {
                        StopTime = GettimeEnd("ETH1");
                        TimepingEth1 = CountTime("ETH1", StartTime, StopTime);
                ...
                if (Eth2 == false)
                {
                    if (Output.Contains("Eth2")) { Eth2 = true; }
                    else
                    {
                        if (i >= 2)
                        {
                ...
                if (Eth3 == false)
                {
                    if (Output.Contains("Eth3")) { Eth3 = true; }
                    else
                    {
                        if (i >= 2)
                        {
                ...
                if (Eth4 == false)
                {
                    if (Output.Contains("Eth4")) { Eth4 = true; }
                    else
                    {
                        if (i >= 2)
                        {
                ...
                if (Server == false)
                {
                    if (Output.Contains("SERVER")) { Server = true; }
                    else
                    {
                        if (i >= 2)
                        {
                            StopTime = GettimeEnd("ETH1");
                            TimepingEth1 = CountTime("ETH1", StartTime, StopTime);
                            ErrorNian("Err_NO_Card_SERVER");
                            MyMessagesBox.Show("\n\nPls! Kiểm tra Card mang SERVER");
                            return false;
                        }
                    }
                }
                if (Eth1 == true & Eth2 == true & Eth3 == true & Eth4 == true & Server == true) { break; }
                delay(2000);
            }
            var nian = Output.Split('\n');
            int total = nian.Length;
            for (int i = 0; i <= total; i++)
            {
                if (nian[i].Contains("Eth1"))
                {
                    output = nian[i];
                    Ethernet1 = GetKeyIDEth(output);
                }
                else if (nian[i].Contains("Eth2"))
                {
                    output = nian[i];
                    Ethernet2 = GetKeyIDEth(output);
                }
                else if (nian[i].Contains("Eth3"))
                {
                    output = nian[i];
                    Ethernet3 = GetKeyIDEth(output);
                }
                else if (nian[i].Contains("Eth4"))
                {
                    output = nian[i];
                    Ethernet4 = GetKeyIDEth(output);
                }
                if (Ethernet1 != "" & Ethernet2 != "" & Ethernet3 != "" & Ethernet4 != "")
                {
                    resultdara = WriteIDEth(Ethernet1, Ethernet2, Ethernet3, Ethernet4);
                    if (resultdara != "FALSE")
                    {
                        return true;
                    }
        ...
        private string WriteIDEth(string Eth1, string Eth2, string Eth3, string Eth4)
        {
            string result = "FALSE";
            string fileSpeed100M = Directory.GetCurrentDirectory() + "\\speed100All.bat";
            string fileSpeed1G = Directory.GetCurrentDirectory() + "\\speed1000All.bat";
            try
            {
                string Speed100M = "echo" + "\n" +
                             "reg add HKEY_LOCAL_MACHINE" + "\\" + "SYSTEM" + "\\" + "CurrentControlSet" + "\\" + "Control" + "\\" + "Class" + "\\" + "{4D36E972-E325-11CE-BFC1-08002BE10318}\\" + Eth1 + " /v *SpeedDuplex /d 4 /t REG_SZ /f" + "\n"
                            + "reg add HKEY_LOCAL_MACHINE" + "\\" + "SYSTEM" + "\\" + "CurrentControlSet" + "\\" + "Control" + "\\" + "Class" + "\\" + "{4D36E972-E325-11CE-BFC1-08002BE10318}\\" + Eth2 + " /v *SpeedDuplex /d 4 /t REG_SZ /f" + "\n"
                            + "reg add HKEY_LOCAL_MACHINE" + "\\" + "SYSTEM" + "\\" + "CurrentControlSet" + "\\" + "Control" + "\\" + "Class" + "\\" + "{4D36E972-E325-11CE-BFC1-08002BE10318}\\" + Eth3 + " /v *SpeedDuplex /d 4 /t REG_SZ /f" + "\n"
                            + "reg add HKEY_LOCAL_MACHINE" + "\\" + "SYSTEM" + "\\" + "CurrentControlSet" + "\\" + "Control" + "\\" + "Class" + "\\" + "{4D36E972-E325-11CE-BFC1-08002BE10318}\\" + Eth4 + " /v *SpeedDuplex /d 4 /t REG_SZ /f" + "\n"
                            + "reg add HKEY_LOCAL_MACHINE" + "\\" + "SYSTEM" + "\\" + "CurrentControlSet" + "\\" + "Control" + "\\" + "Class" + "\\" + "{4D36E972-E325-11CE-BFC1-08002BE10318}\\" + Eth4 + " /v LinkSpeed /d 32 /t REG_SZ /f" + "\n"
                            + "stop";
                string Speed1G = "echo" + "\n" +
                                 "reg add HKEY_LOCAL_MACHINE" + "\\" + "SYSTEM" + "\\" + "CurrentControlSet" + "\\" + "Control" + "\\" + "Class" + "\\" + "{4D36E972-E325-11CE-BFC1-08002BE10318}\\" + Eth1 + " /v *SpeedDuplex /d 0 /t REG_SZ /f" + "\n"
                                + "reg add HKEY_LOCAL_MACHINE" + "\\" + "SYSTEM" + "\\" + "CurrentControlSet" + "\\" + "Control" + "\\" + "Class" + "\\" + "{4D36E972-E325-11CE-BFC1-08002BE10318}\\" + Eth2 + " /v *SpeedDuplex /d 0 /t REG_SZ /f" + "\n"
                                + "reg add HKEY_LOCAL_MACHINE" + "\\" + "SYSTEM" + "\\" + "CurrentControlSet" + "\\" + "Control" + "\\" + "Class" + "\\" + "{4D36E972-E325-11CE-BFC1-08002BE10318}\\" + Eth3 + " /v *SpeedDuplex /d 0 /t REG_SZ /f" + "\n"
                                + "reg add HKEY_LOCAL_MACHINE" + "\\" + "SYSTEM" + "\\" + "CurrentControlSet" + "\\" + "Control" + "\\" + "Class" + "\\" + "{4D36E972-E325-11CE-BFC1-08002BE10318}\\" + Eth4 + " /v *SpeedDuplex /d 0 /t REG_SZ /f" + "\n"
                                + "reg add HKEY_LOCAL_MACHINE" + "\\" + "SYSTEM" + "\\" + "CurrentControlSet" + "\\" + "Control" + "\\" + "Class" + "\\" + "{4D36E972-E325-11CE-BFC1-08002BE10318}\\" + Eth4 + " /v LinkSpeed /d 59 /t REG_SZ /f" + "\n"
                                + "stop";
                FileStream fs;
                fs = new FileStream(fileSpeed100M, FileMode.Create);
                StreamWriter write = new StreamWriter(fs, Encoding.UTF8);
                write.WriteLine(Speed100M);
                write.Flush();
                fs.Close();

                FileStream fs1;
                fs1 = new FileStream(fileSpeed1G, FileMode.Create);
                StreamWriter write1 = new StreamWriter(fs1, Encoding.UTF8);
                write1.WriteLine(Speed1G);
                write1.Flush();
                fs1.Close();
                result = "PASS";
                return result;
            ...
            {
                MyMessagesBox.Show("WriteIDEth_Error");
                return result;
            }
            return result;
        }
        private bool EnabelEthAll()
        {
            bool Resutl = false;
            //cmn.openExe1(pathfile1, "enableEthAll-SV");
            ////double TimeStart = GettimeStart("Check_Enabel_All_Eth");
            //delay(8000);
            bool Eth1 = true;
            bool Eth2 = false;
            bool Eth3 = false;
            ...
            for (int i = 0; i <= 5; i++)
            {
                ...
                if (Eth1 == false)
                {
                    ...
                    if (statusEth1.Contains("disconnected"))
                    {
                        if (i == 2)
                        {
                ...
                if (Eth2 == false)
                {
                    ...
                    if (statusEth2.Contains("disconnected"))
                    {
                        if (i == 2)
                        {
                ...
                if (Eth3 == false)
                {
                    ...
                    if (statusEth3.Contains("disconnected"))
                    {
                        if (i == 2)
                        {
                ...
                if (Eth4 == false)
                {
                    ...
                    if (statusEth4.Contains("disconnected"))
                    {
                        if (i == 2)
                        {
            ...
            //double TimeEnd = GettimeEnd("Check_Enabel_All_Eth");
            //statusEth = CountTime("Check_Enabel_All_Eth", TimeStart, TimeEnd);
            return Resutl;
        }
        ...
        private void Summary_test()
        {
            string PEth1 = "";
            if (checkPingEth1Pass == true)
            {
                PEth1 = "Ping_Eth1: PASS";
            }
            else
            {
                PEth1 = "Ping_Eth1: FAIL";
            }
            string PEth2 = "";
            if (checkPingEth2Pass == true)
            {
                PEth2 = "Ping_Eth1: PASS";
            }
            else
            {
                PEth2 = "Ping_Eth1: FAIL";
            }
            msg("Summary_test: \n\r" + PEth1 + "\n\r" + PEth2);
        }
        ...
        private void button20_Click(object sender, EventArgs e)
        {
            ...
            //int SNdemo = random.Next(1000000, 9999999);
            //int Tdemo = random.Next(500, 700);
            //string directoryPath = "D:\\LogFile" + @"\" + date + "\\" + ModelnameFile;
            //if (txtOutput.InvokeRequired) { txtOutput.Invoke(new Action(() => Data = txtOutput.Text)); }
            //else { Data = txtOutput.Text; }
            //if (!System.IO.Directory.Exists(directoryPath))
            ...
            //    System.IO.Directory.CreateDirectory(directoryPath);
            //}
            //string filePath = directoryPath + @"\" + SNdemo + "_" + "PASS" + "_" + Tdemo + "_" + Type_Program + ".txt";
            //pathsaveftp = filePath;
            //StreamWriter strWrite = new StreamWriter(filePath, true);
            //strWrite.WriteLine(Data);
            //strWrite.Flush();
            //strWrite.Close();
            //delay(100);
            ////string filePath = @"D:\LogFile\16.11.2023\COM\Test_FAIL_349.txt";
            //pathsaveftp = filePath;
            //sModem = "CGM4982COM";
        ...
        {
            count = 300;
            //if (sModem == "CGM4981COM" | sModem == "CGM4981COX")
            //{
            //    count = 280;
            //}
            // if (count240s == true)
        ...
        private void rFQA11ToolStripMenuItem_DoubleClick(object sender, EventArgs e)
        {

        }

        private void cmbcomzble_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void fanCheckToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (!btnOpenCom.Text.Contains("Disconnect") | cmmType.SelectedIndex == 1)
            {
                MyMessagesBox.Show("\n\n              Setting Program!!!");
            ...
            if (!Check_SN(lbsndut.Text.Trim())) { StatusERR("Check_SN"); return; }
            delay(500);
            Send_ErrCode("ERRFAN", "Error_Fan_Check");
            delay(2000);
            Send_ErrCode("ERRFAN", "Error_Fan_Check");
            LogFile("FAIL", "ERRFAN", "");
            LogFileFail("Error_Fan_Check", "ERRFAN");
            LogFilePassNian("FAIL", "ERRFAN");
        }
        ...
        private void sendsftpToolStripMenuItem_Click(object sender, EventArgs e)
        {
            cmn.openExe1(pathfile1, "disableEthAll-SV");
            delay(1000);
            string Data1 = "";
            ModelnameFile = "XB8OPC00";
            string date = DateTime.Now.ToString("dd-MM-yyyy");
            date = date.Replace("-", ".");
            string directoryPath = "D:\\LogFiletry" + @"\" + date + "\\" + ModelnameFile;
            string sns = "TEST_PROGRAM1";
            string passFail = "PASS";
            Random times = new Random();
            int timed = times.Next(100, 999);
            string time11 = timed.ToString();
            string filePath = directoryPath + @"\" + sns + "_" + passFail + "_" + time11 + "_" + Type_Program + ".txt";
            pathsaveftp = filePath;
            if (txtOutput.InvokeRequired) { txtOutput.Invoke(new Action(() => Data1 = txtOutput.Text)); }
            else { Data1 = txtOutput.Text; }
        ...
        private void sPVOIPToolStripMenuItem_Click(object sender, EventArgs e)
        {
            BugVOIPConal1();
        }

        private void sPVOIP2ToolStripMenuItem_Click(object sender, EventArgs e)
        {
            BugVOIPConal2();
        }

        private void fVOI10ToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void wPSToolStripMenuItem_Click(object sender, EventArgs e)
        {
            comCmd("out9_on");
            delay(5000);
            comCmd("out9_off");
        ...
        private void checkbox_FQA2_CheckedChanged(object sender, EventArgs e)
        {
        ...
        {
            string password_unlock = "";
            Inputpassword.Show("Unlocked: ");
            password_unlock = Inputpassword.password;
            if (password_unlock == "qEvn123!")
            {
                lbl_debug_P.Visible = true;
                lbl_debug_F.Visible = true;
                lbl_pass_debug.Visible = true;
                lbl_fail_debug.Visible = true;
                Unlocked = true;
                Log.Save_Logs_File_status_program("Unlocked", ma_loi, Inputpassword.id);
                btn_unlock.Visible = false;
                Check_lock_may.Text = "Lock";
                MsBox.Show("Chương trình sẽ ở chế độ Lock sau sửa chữa \r\nThank you very much!!");
                btnStart.Enabled = true;

            }
            else if (password_unlock == "huyen")
            {
                Log.Save_Logs_File_status_program("Unlocked_DC", ma_loi, Inputpassword.id);
                btn_unlock.Visible = false;
                //Check_lock_may.Text = "Lock";
                try
                {
                ...
                MessageBox.Show("Unlock đặc biệt \r\n Download lại để test ");

            }
            else
            {
                Log.Save_Logs_File_status_program("Locked - Wrong Password", ma_loi, Inputpassword.id);
                MessageBox.Show("Sai mật khẩu >>> Gọi kỹ sư ");
            }
        }

        private void getMIBToolStripMenuItem_Click(object sender, EventArgs e)
        {

        }

        private void sNToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.4.1.46366.4292.80.1.3.0");
            msg(cable.value);
        ...
        {
            cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.2.1.1.1.0");
            msg(cable.value);
        }

        private void sSIDToolStripMenuItem_Click(object sender, EventArgs e)
        {
            cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.4.1.17270.50.2.2.2.1.1.3.10001");
            msg(cable.value);
        }

        private void wFPasswordToolStripMenuItem_Click(object sender, EventArgs e)
        {
            cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.4.1.17270.50.2.2.3.1.1.2.10001");
            msg(cable.value);
        }

        private void mACBLEToolStripMenuItem_Click(object sender, EventArgs e)
        {
            cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.4.1.46366.4292.80.2.9.0");
            msg(cable.value);
        }

        private void cMMACToolStripMenuItem_Click(object sender, EventArgs e)
        {
            cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.4.1.46366.4292.80.1.4.0");
            msg(cable.value);
        }

        private void wANMACToolStripMenuItem_Click(object sender, EventArgs e)
        {
            cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.2.1.2.2.1.6.1");
            msg(cable.value);
        }

        private void mTAMACToolStripMenuItem_Click(object sender, EventArgs e)
        {
            cable.connection(CMip.Trim(), 1, "private", "1.3.6.1.2.1.2.2.1.6.16");
            msg(cable.value);
        }
